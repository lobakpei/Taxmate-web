...
function transactionFacts(type,id,amountMinor,date,description,evidenceRef,entityId,accounts){
  ...
  else if(type===CompanyLedger.TYPES.COMPANY_EXPENSE){Object.assign(facts,{invoicePartyId:entityId,payerPaymentAccountId:accounts.company.id,payerOwnerType:'entity',category:'ordinary_running',treatmentBasis:'ordinary_running_expense_confirmed'});facts.companyTaxTreatment=CompanyTreatment.assessExpense(facts,accounts.profile,ordinary);}
  else if(type===CompanyLedger.TYPES.PERSONALLY_PAID_EXPENSE){Object.assign(facts,{invoicePartyId:entityId,payerPaymentAccountId:accounts.personal.id,payerOwnerType:'person',reimbursementExpected:true,category:'ordinary_running',treatmentBasis:'ordinary_running_expense_confirmed'});facts.companyTaxTreatment=CompanyTreatment.assessExpense(facts,accounts.profile,ordinary);}
