...
async function run(){
 ...
 await rootUI.locator('.tm-info-dialog').waitFor();
  const infoGeometry=await rootUI.locator('.tm-info-dialog').evaluate(node=>{const rect=node.getBoundingClientRect(),active=document.activeElement,style=getComputedStyle(node);return{centerDeltaX:Math.abs((rect.left+rect.width/2)-innerWidth/2),centerDeltaY:Math.abs((rect.top+rect.height/2)-innerHeight/2),focused:node.contains(active),scrim:getComputedStyle(node.closest('.tm-info-dialog-scrim')).backgroundColor,borderRadius:style.borderRadius,overflow:style.overflow};});
 assert.ok(infoGeometry.centerDeltaX<2&&infoGeometry.centerDeltaY<2,`information dialog is not centred: ${JSON.stringify(infoGeometry)}`);
  ...
  assert.equal(infoGeometry.scrim,'rgba(15, 22, 32, 0.32)','information dialog keeps the app visible behind its lighter backdrop');
  assert.equal(infoGeometry.borderRadius,'20px','information dialog keeps four matching rounded corners');
  assert.equal(infoGeometry.overflow,'hidden','information dialog footer cannot square off the lower corners');
  assert.equal(await rootUI.locator('.tm-info-dialog .tm-grab,.tm-info-dialog .tm-dialog-close').count(),0,'information dialog has no redundant drag handle or close cross');
  await page.screenshot({path:path.join(evidence,'information-dialog-'+label+'.png')});
  await rootUI.locator('.tm-info-dialog-scrim').evaluate(node=>node.dispatchEvent(new MouseEvent('click',{bubbles:true})));
  assert.equal(await rootUI.locator('.tm-info-dialog').count(),1,'backdrop does not add a second undisclosed close route');
 ...
 await rootUI.getByRole('button',{name:'Leave setup',exact:true}).click();
 ...
 assert.equal(await rootUI.getByRole('button',{name:'Leave setup',exact:true}).count(),1,'Step 1 has one clear setup exit after continuing setup');
