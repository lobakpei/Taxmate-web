...
const currentCache='taxmate-v2-founder-brand-sync-runtime-integrity-1';
...
const cases=[],externalRequests=[],sentryRequests=[],contexts=new Set(),servers=new Set();
let assertions=0;
...
async function launch(port,raw=pendingRaw){
  const profile=path.join(os.tmpdir(),`tm-sync-${port}-${Date.now().toString(36)}`);
  ...
  await context.addInitScript(value=>{localStorage.setItem('taxmateuk_sync_outbox_v1',value);localStorage.setItem('tmOnboardDone','1');localStorage.setItem('taxmateuk_analytics_consent','denied');},raw);
...
async function serviceWorkerLifecycle(){
  ...
  let port=4285,control=await startServer(port,'old-worker'),browser=await launch(port,pendingRaw),page=browser.page;
  ...
  port=4286;control=await startServer(port,'normal');browser=await launch(port,pendingRaw);page=browser.page;
  ...
  await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>!!navigator.serviceWorker.controller);equal(await controllerIdentity(page),null,'coherent current worker replaces legacy worker');
  ...
  await browser.context.setOffline(true);await page.reload({waitUntil:'domcontentloaded'});const offline=await page.evaluate(()=>eval("({runtime:{...SYNC_RUNTIME},raw:localStorage.getItem(SYNC_OUTBOX_KEY),status:syncStatus(),controller:!!navigator.serviceWorker.controller})"));equal(offline.runtime.blocked,false,'offline reopen uses coherent app and sync runtime');equal(offline.raw,pendingRaw,'offline installed-PWA reopen preserves exact pending outbox bytes');check(offline.controller,'offline reopen remains service-worker controlled');
  ...
  cases.push({name:'service-worker-atomic-offline',status:'PASS',failedKeys,installedKeys,assetFallback,rawSha256:sha(pendingRaw)});await browser.context.close();await new Promise(resolve=>control.server.close(resolve));servers.delete(control.server);process.stdout.write('PASS service-worker-atomic-offline\n');
}
...
(async()=>{
  ...
  const result={status:'PASS',generatedAt:new Date().toISOString(),assertions,cases,externalRequests,sentryRequests,identity:require('../../src/core/versions').VERSIONS,sourceSha256:{app:sha(fs.readFileSync(path.join(root,'src/app/app.js'))),sync:sha(fs.readFileSync(path.join(root,'src/core/sync.js'))),sw:sha(fs.readFileSync(path.join(root,'sw.js')))}};fs.writeFileSync(resultPath,JSON.stringify(result,null,2));process.stdout.write(`SYNC_RUNTIME_BROWSER_PASS assertions=${assertions}\n`);
})().catch(error=>{fs.mkdirSync(evidence,{recursive:true});fs.writeFileSync(resultPath,JSON.stringify({status:'FAIL',generatedAt:new Date().toISOString(),assertions,cases,externalRequests,sentryRequests,error:error.stack||String(error)},null,2));console.error(error.stack||error);process.exitCode=1;}).finally(async()=>{for(const context of [...contexts])try{await context.close();}catch(_){}for(const server of [...servers])await new Promise(resolve=>server.close(resolve));});
