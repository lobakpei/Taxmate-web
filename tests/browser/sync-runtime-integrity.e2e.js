...
async function launch(port,options={}){
  ...
  if(options.seed!==false)await context.addInitScript(value=>{localStorage.setItem('taxmateuk_account_v1:local:sync-outbox',value);localStorage.setItem('taxmateuk_account_v1:local:onboarding-done','1');localStorage.setItem('taxmateuk_analytics_consent','denied');},raw);
...
async function serviceWorkerLifecycle(){
  ...
  await browser.context.setOffline(false);await page.evaluate(()=>window.dispatchEvent(new Event('online')));equal(await page.evaluate(()=>localStorage.getItem(SYNC_OUTBOX_KEY)),pendingRaw,'reconnect alone cannot discard or rewrite a pending outbox');
