...
test('stale Storage path safely falls back to the same record URL and remains fail-closed in the archive',async()=>{
  ...
  const rows=await Backup.collectReceipts({state,user:{uid:'u'},storageUrl:async()=>{throw Object.assign(new Error('missing'),{code:'storage/object-not-found'});},download});
...
test('missing references and download failures have distinct safe categories and messages',async()=>{
  const missing=await Backup.collectReceipts({state:baseState([entry('e1','receipts/u/missing.jpg',null)]),user:{uid:'u'},storageUrl:async()=>{throw Object.assign(new Error('private path'),{code:'storage/object-not-found'});},download}).then(()=>null,error=>error);
...
test('offline/auth failures remain distinct from an HTTP receipt failure',async()=>{
  ...
  const unauthenticated=await Backup.collectReceipts({state:baseState([entry('e1','receipts/u/private.jpg',null)]),user:{uid:'u'},storageUrl:async()=>{throw Object.assign(new Error('sign in'),{code:'storage/unauthenticated'});},download}).then(()=>null,error=>error);
