...
test('app guards every outbox write and convergence path behind the runtime lock',()=>{
  ...
  // Same blocked-runtime state; the message is now a localised key.
  assert.match(app,/state:'update-required'[\s\S]{0,160}sync\.updateRequired/);
  assert.match(require('../../scripts/i18n-audit').I18N.en['sync.updateRequired'],/Local data is safe/);
  assert.match(app,/function flushSyncOutbox\(reason\)\{\s*if\(ACCOUNT_TRANSITION_PENDING\|\|CLOUD\.deletionBlocked\|\|CLOUD\.retentionBlocked\|\|CLOUD\.controlsCached\|\|CLOUD\.firstSyncBlocked\)[\s\S]{0,240}if\(SYNC_RUNTIME\.blocked\)/);
  ...
  assert.match(app,/function startUserSync\(u,options=\{\}\)\{\s*if\(ACCOUNT_TRANSITION_PENDING\|\|CLOUD\.deletionBlocked\|\|CLOUD\.retentionBlocked\|\|CLOUD\.controlsCached\)[\s\S]{0,260}if\(SYNC_RUNTIME\.blocked\)/);
