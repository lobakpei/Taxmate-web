...
test('favicon fills the small canvas with a legible face while unchanged PWA assets retain safe bounds',()=>{
  ...
  for(const [file,value] of Object.entries(assetManifest.iconRasterValidation)){const width=value.foregroundBoundsRatio.right-value.foregroundBoundsRatio.left,small=value.width<=48;assert.ok(width>=(small?0.84:0.75)&&width<=(small?0.94:0.83),`${file} optical foreground width ${width}`);if(!small)assert.ok(value.foregroundBounds.left>0&&value.foregroundBounds.top>0&&value.foregroundBounds.right<value.width-1&&value.foregroundBounds.bottom<value.height-1,`${file} full-size foreground remains uncropped`);assert.ok(value.blackPixels>=(value.width<=16?2:3),`${file} retains legible black eye and mouth pixels`);}
...
test('all website identity metadata points to the new assets while product SEO copy stays unchanged',()=>{
  ...
  assert.match(home,/rel="apple-touch-icon" sizes="180x180" href="\/apple-touch-icon\.png\?v=20260903-6"/);
...
test('manifest, service worker and Hosting build carry every production brand asset',()=>{
  assert.deepEqual(manifest.icons.map(icon=>[icon.src,icon.sizes,icon.purpose]),[['icon-192.png?v=20260903-6','192x192','any'],['icon-512.png?v=20260903-6','512x512','any'],['icon-512-maskable.png?v=20260903-6','512x512','maskable']]);
  for(const file of ['favicon-16x16.png','favicon-32x32.png','favicon-48x48.png','favicon.ico','apple-touch-icon.png','icon-192.png','icon-512.png','icon-512-maskable.png']){assert.ok(sw.includes(`/${file}?v=20260903-6`),`${file} versioned and precached`);assert.ok(build.includes(`'${file}'`),`${file} copied`);}
