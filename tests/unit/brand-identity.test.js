...
test('transparent favicon stays centred and legible while deep-background PWA assets retain safe bounds',()=>{
  for(const [file,size] of Object.entries({'favicon-16x16.png':16,'favicon-32x32.png':32,'favicon-48x48.png':48,'apple-touch-icon.png':180,'icon-192.png':192,'icon-512.png':512,'icon-512-maskable.png':512})){
    const dimensions=pngDimensions(file);assert.deepEqual({width:dimensions.width,height:dimensions.height},{width:size,height:size},file);assert.equal(dimensions.bitDepth,8);assert.equal(dimensions.colourType,file.startsWith('favicon-')?6:2,`${file} colour mode`);
  }
  for(const [file,value] of Object.entries(assetManifest.iconRasterValidation)){const width=value.foregroundBoundsRatio.right-value.foregroundBoundsRatio.left,small=value.width<=48;assert.ok(width>=(small?0.74:0.63)&&width<=(small?0.90:0.72),`${file} optical foreground width ${width}`);assert.ok(value.foregroundBounds.left>0&&value.foregroundBounds.top>0&&value.foregroundBounds.right<value.width-1&&value.foregroundBounds.bottom<value.height-1,`${file} foreground remains centred and uncropped`);if(small){assert.ok(value.transparentPixels>0,`${file} has no white outer frame`);assert.ok(value.blackPixels>=(value.width<=16?2:3),`${file} retains legible black eye and mouth pixels`);}}
  const safe=assetManifest.maskableValidation;assert.equal(safe.transparentPixels,0);assert.equal(safe.opaquePixels,512*512);assert.ok(safe.foregroundBoundsRatio.left>=0.18&&safe.foregroundBoundsRatio.top>=0.18&&safe.foregroundBoundsRatio.right<=0.82&&safe.foregroundBoundsRatio.bottom<=0.82);assert.ok(safe.foregroundBoundsRatio.right-safe.foregroundBoundsRatio.left>0.45);
...
test('all website identity metadata points to the new assets while product SEO copy stays unchanged',()=>{
  ...
  assert.match(home,/rel="apple-touch-icon" sizes="180x180" href="\/apple-touch-icon\.png\?v=20260903-8"/);
...
test('manifest, service worker and Hosting build carry every production brand asset',()=>{
  assert.equal(manifest.background_color,'#0F1620');assert.equal(manifest.theme_color,'#0F1620');
  assert.deepEqual(manifest.icons.map(icon=>[icon.src,icon.sizes,icon.purpose]),[['icon-192.png?v=20260903-10','192x192','any'],['icon-512.png?v=20260903-10','512x512','any'],['icon-512-maskable.png?v=20260903-10','512x512','maskable']]);
  for(const file of ['favicon-16x16.png','favicon-32x32.png','favicon-48x48.png','apple-touch-icon.png']){assert.ok(sw.includes(`/${file}?v=20260903-8`),`${file} stable SEO identity remains precached`);assert.ok(build.includes(`'${file}'`),`${file} copied`);}for(const file of ['icon-192.png','icon-512.png','icon-512-maskable.png']){assert.ok(sw.includes(`/${file}?v=20260903-10`),`${file} PWA release identity is precached`);assert.ok(build.includes(`'${file}'`),`${file} copied`);}assert.ok(sw.includes("'/favicon.ico'"));assert.ok(build.includes("'favicon.ico'"));
