...
test('manifest, service worker and Hosting build carry every production brand asset',()=>{
  ...
  assert.deepEqual(manifest.icons.map(icon=>[icon.src,icon.sizes,icon.purpose]),[['icon-192.png?v=20260903-8','192x192','any'],['icon-512.png?v=20260903-8','512x512','any'],['icon-512-maskable.png?v=20260903-8','512x512','maskable']]);
  for(const file of ['favicon-16x16.png','favicon-32x32.png','favicon-48x48.png','apple-touch-icon.png','icon-192.png','icon-512.png','icon-512-maskable.png']){assert.ok(sw.includes(`/${file}?v=20260903-8`),`${file} versioned and precached`);assert.ok(build.includes(`'${file}'`),`${file} copied`);}assert.ok(sw.includes("'/favicon.ico'"));assert.ok(build.includes("'favicon.ico'"));
