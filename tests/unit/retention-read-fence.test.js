...
test('LTD download acceptance rejects old and missing epochs and locks while purge is pending',()=>{
 const context=vm.createContext({CLOUD:{retentionControl:{epoch:2},resetControl:{status:'complete',resetEpoch:3},retentionBlocked:false},currentAccountResetEpoch:()=>3,activeAccountUid:()=> 'demo-owner',explicitOwner:x=>x&&x.accountOwnerUid,console});vm.runInContext(fn('function ltdCloudForAccount(','function pendingLtdRecoveryEnvelopes('),context);
 const current={retentionEpoch:2,accountResetEpoch:3},result=context.ltdCloudForAccount(null,[{retentionEpoch:1,accountResetEpoch:3},{retentionEpoch:2},{retentionEpoch:2,accountResetEpoch:2},current], 'test');assert.equal(result.envelopes.length,1);assert.equal(result.envelopes[0].retentionEpoch,2);assert.equal(result.envelopes[0].accountResetEpoch,3);
...
test('an LTD read started before epoch or account generation changes cannot mutate canonical state',async()=>{
 for(const changed of ['epoch','generation','pending']){
  let release,writes=0;const pending=new Promise(resolve=>{release=resolve;}),empty={forEach:()=>{}},context=vm.createContext({CLOUD:{generation:1,retentionControl:{epoch:1},retentionBlocked:false},ltdAccessDecision:()=>({allowed:true}),userRoot:()=>({collection:()=>({doc:()=>({get:async()=>({exists:false})}),get:async()=>empty})}),TaxMateLtdSync:{COLLECTIONS:['companyProfiles'],validateEnvelope:()=>{},docId:value=>value},ltdCollectionRef:()=>({}),retentionQuery:()=>({get:async()=>{await pending;return empty;}}),syncGenerationCurrent:(_uid,g)=>context.CLOUD.generation===g,ltdCloudForAccount:()=>({envelopes:[],anchor:null}),validateLtdAnchorConsistency:()=>{},setLtdRemote:()=>{writes++;},reconcileLtdState:()=>{writes++;return{};}});
...
test('auth transition rechecks the account after asynchronous receipt-reset cleanup',async()=>{
 ...
 const context=vm.createContext({AUTH_TRANSITION_GENERATION:0,ACTIVE_ACCOUNT_SCOPE:{kind:'local'},S:{tab:'home'},firebase:{auth:()=>auth},TaxMateAccountStorage:{activeUidMatches:()=>false,firebaseScope:uid=>({kind:'firebase',uid})},beginAccountTransition:()=>{},readAccountControls:async()=>({reset:{status:'complete',resetEpoch:2},retention:null}),applyServerResetWithReceipts:async()=>{started=true;await pause;},activateAccountScope:()=>{activated++;throw Error('Old account must not be activated');},safeActionTrace:()=>{}});
...
test('superseded control refresh cannot reactivate the old account or clear the new account refresh promise',async()=>{
 for(const rejectCleanup of [false,true]){
  ...
  const context=vm.createContext({AUTH_TRANSITION_GENERATION:1,CLOUD:{controlsCached:true,controlsRefreshPromise:null},S:{tab:'home'},localStorage:{},cloudUser:()=>auth.currentUser,cachedAccountControls:()=>null,TaxMateAccountStorage:{firebaseScope:uid=>({kind:'firebase',uid}),read:()=>null},TaxMateRetentionPolicy:{controlWritable:()=>true},renderSyncStatus:()=>{},beginAccountTransition:()=>{},readAccountControls:async()=>({reset:{status:'complete',resetEpoch:2},retention:null}),applyServerResetWithReceipts:async()=>{started=true;await pause;if(rejectCleanup)throw Error('old cleanup failed after account switch');},activateAccountScope:()=>{activated++;throw Error('Old account must not be activated');}});
  vm.runInContext(fn('async function refreshCachedAccountControls(options={})','function onboardingDoneFlag('),context);const running=context.refreshCachedAccountControls();for(let n=0;n<10&&!started;n++)await Promise.resolve();assert.equal(started,true);
...
test('leaving a partnership updates the current canonical business after asynchronous cloud replacement',async()=>{
 for(const changedAccount of [false,true]){
  ...
  const context=vm.createContext({AUTH_TRANSITION_GENERATION:1,DEVICE_ID:'device-a',FB:{subs:{SHARED88:[]}},bizById:()=>state.business,cloudUser:()=>auth,currentAccountResetEpoch:()=>4,callSecureFunction:async(_name,data)=>{assert.equal(data.accountResetEpoch,4);return pause;},TaxMateSync:{touch:b=>({...b,updatedAt:123,deviceId:'device-a'})},save:()=>{saved++;},paintSync:()=>{},render:()=>{}});
...
test('an offline control read that completes after reconnect resumes server verification without waiting for another online event',async()=>{
 for(const online of [false,true]){
  ...
  const context=vm.createContext({AUTH_TRANSITION_GENERATION:0,ACTIVE_ACCOUNT_SCOPE:{kind:'local'},ACCOUNT_TRANSITION_PENDING:false,ACCOUNT_SCOPE_NORMALIZATION_PENDING:false,STORE_KEY:'canonical',S:{tab:'home',businesses:[]},CLOUD:{},navigator:{onLine:online},firebase:{auth:()=>auth},TaxMateAccountStorage:{activeUidMatches:(scope,uid)=>scope?.kind==='firebase'&&scope.uid===uid,firebaseScope:uid=>({kind:'firebase',uid}),localAssociationPending:()=>false,localAssociationState:()=>null},localStorage:{getItem:()=>null,setItem:()=>{}},load:()=>({tab:'home',businesses:[]}),metaSyncSnapshot:()=>({}),loadSyncOutbox:()=>({items:[]}),beginAccountTransition:()=>{},readAccountControls:async()=>{throw Object.assign(Error('offline read completed after reconnect'),{code:'unavailable'});},cachedAccountControls:()=>({reset:null,retention:null}),accountControlConnectivityFailure:()=>true,activateAccountScope:()=>{},applyRetentionControlLocally:()=>{},installRetentionWatcher:()=>{},safeActionTrace:()=>{},activeLtdProfile:()=>null,renderBackgroundAccount:()=>{},renderSyncStatus:()=>{},refreshCachedAccountControls:async()=>{refreshed++;},handleSyncListenerError:()=>{}});
...
test('network failure during control refresh restores the pending zero-write account confirmation',async()=>{
 let confirmations=0;const user={uid:'owner-a'},context=vm.createContext({AUTH_TRANSITION_GENERATION:1,CLOUD:{controlsCached:true,controlsRefreshPromise:null},S:{tab:'home',businesses:[]},localStorage:{},cloudUser:()=>user,cachedAccountControls:()=>({reset:null,retention:null}),TaxMateAccountStorage:{firebaseScope:uid=>({kind:'firebase',uid}),localAssociationPending:()=>true,localAssociationState:()=>({status:'confirmation',cloudState:'empty'}),localAssociationTargets:()=>true},clearUserSyncListeners:()=>{},beginAccountTransition:()=>{},readAccountControls:async()=>{throw Object.assign(Error('offline'),{code:'unavailable'});},activeLtdProfile:()=>null,accountControlConnectivityFailure:()=>true,reportSyncErrorOnce:()=>{},scheduleAccountControlRetry:()=>{},safeActionTrace:()=>{},renderSyncStatus:()=>{},renderBackgroundAccount:()=>{},setFirstSyncConfirmation:(_user,state,_correlation,error)=>{assert.equal(state,'empty');assert.equal(error,true);confirmations++;}});
 vm.runInContext(fn('async function refreshCachedAccountControls(options={})','function onboardingDoneFlag('),context);assert.equal(await context.refreshCachedAccountControls(),false);assert.equal(confirmations,1);assert.equal(context.CLOUD.controlsCached,true);assert.equal(context.CLOUD.firstSyncBlocked,true);
});
