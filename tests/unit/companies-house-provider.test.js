...
test('Founder Preview alias gate requires loopback, demo Firebase emulators and explicit mode together',()=>{
  assert.equal(Provider.founderPreviewAllowed(enabled),true);
  for(const context of [
    {...enabled,hostname:'www.taxmate.uk'},
    {...enabled,firebaseProjectId:'taxmate-uk-2'},
    {...enabled,firebaseEmulators:false},
    {...enabled,previewMode:''}
  ])assert.equal(Provider.founderPreviewAllowed(context),false,JSON.stringify(context));
});

test('enabled alias returns only the fictional fixture and makes zero fallback provider calls',async()=>{
  let calls=0;const fallback={isNetworkProvider:true,async lookup(){calls++;return{status:'not_found'};}},provider=Provider.createFounderPreviewProvider(fallback,enabled);
  ...
  assert.deepEqual(result.company,Provider.FOUNDER_PREVIEW_COMPANY);
...
test('disabled alias is rejected without fallback provider call',async()=>{
  let calls=0;const provider=Provider.createFounderPreviewProvider({isNetworkProvider:true,async lookup(){calls++;return{status:'found'};}},{...enabled,hostname:'www.taxmate.uk'});
