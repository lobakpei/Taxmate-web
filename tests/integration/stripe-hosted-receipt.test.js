...
const stripe=new Stripe(process.env.STRIPE_SECRET_KEY);const sessionId=process.env.STRIPE_HOSTED_SESSION_ID;const expectedUid=process.env.STRIPE_HOSTED_UID;
...
test('completed hosted Stripe TEST Checkout reaches backend entitlement truth',async()=>{
  assert.equal(process.env.TAXMATE_STRIPE_SANDBOX,'acct_1U671tDl7HCNqvcV');assert.match(sessionId||'',/^cs_test_/);assert.match(expectedUid||'',/^staging-hosted-/);
  const session=await stripe.checkout.sessions.retrieve(sessionId,{expand:['line_items','subscription']});
  assert.equal(session.status,'complete');assert.equal(session.payment_status,'paid');assert.equal(session.currency,'gbp');assert.equal(session.amount_total,849);assert.equal(session.total_details.amount_tax,0);assert.equal(session.automatic_tax.enabled,false);assert.equal(session.line_items.data[0].price.id,'price_1U673zDl7HCNqvcVI2CIiX6w');
