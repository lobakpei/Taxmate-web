...
test('foreign receipt path is blocked before every Storage or URL request even when a fallback URL exists',async()=>{
  ...
  assert.deepEqual(Backup.diagnostic(error),{category:'foreign_receipt_reference',code:'BACKUP_FOREIGN_RECEIPT_BLOCKED',count:1,stage:'receipt_owner'});assert.equal(storageCalls,0);assert.equal(downloads,0);assert.equal(foreign,1);
});
