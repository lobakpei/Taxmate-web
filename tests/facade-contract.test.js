...
test('facade exposes the complete stable semantic callback surface',()=>{
  const {facade}=make();assert.equal(CALLBACKS.length,69);for(const callback of ['onDiscardCompanySetup','onSaveStatutoryReview','onRecheckCompaniesHouse','onCreateSalesInvoice','onCorrectSalesInvoice','onVoidSalesInvoice','onCreateSupplierBill','onCorrectSupplierBill','onVoidSupplierBill','onRegisterCompanyAsset','onCorrectCompanyAsset','onVoidCompanyAsset','onReverseCompanyBookPayment','onReverseCompanyDepreciation','onMatchBankStatement','onDeleteBankReconciliation','onPrepareCompanyYear','onDownloadSelfFilingPack'])assert.ok(CALLBACKS.includes(callback));for(const callback of CALLBACKS)assert.equal(typeof facade[callback],'function',callback);const snapshot=facade.getSnapshot();assert.deepEqual(snapshot.callbacks,CALLBACKS);assert.equal(snapshot.contractVersion,'taxmate-ltd-ui-facade.3');assert.equal(snapshot.packageStatus,'INTEGRATED_CANDIDATE');assert.equal(snapshot.runtime.externalNetwork,false);assert.equal(snapshot.runtime.firebase,false);assert.equal(JSON.stringify(snapshot).includes('<div'),false);
});
