...
| REAL_DURABLE | Home / Settings | Install TaxMate promotion + Settings download | `doInstall` + PWA install policy | native browser PWA prompt or iOS guidance | installed PWA where supported | Free | installed state survives reload; Not now lasts 14 days | hidden for empty/installed states; device-specific guidance | Keep |
