...
- Stripe LIVE products, monthly/yearly prices and the production webhook exist, and the webhook secret is stored. The newly created restricted key has never been used, but its one-time value was not retained; production `STRIPE_SECRET_KEY` remains absent pending Founder-authorised rotation. Paid Checkout therefore remains fail-closed and no changed Functions candidate has been deployed.
- `HKGER`, `EVRI` and `WORCESTER` exist only as inactive production Firestore placeholders with no tier, duration/expiry or maximum-redemption values. They cannot be redeemed until the Founder supplies those values and deliberately runs the protected admin command.
