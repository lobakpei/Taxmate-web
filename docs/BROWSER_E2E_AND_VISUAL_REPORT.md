...
The current build preserves the frozen layout; changes after the prior responsive/language matrix are legal wording, partnership behavior, App Check transport and PWA cache correctness rather than a redesign. Current browser checks reconfirm Google-only UI, exact pricing, monthly-only launch wording, analytics off/no GA loader before consent, matching Terms/Privacy, public-email allow-list and all five primary pages. The existing 320–1920 responsive, six-language, RTL, light/dark and dialog evidence remains applicable. A real server-stop/offline-reopen test now passes with `2026-08-19.production-readiness-rc.1` rather than an older retained cache.
