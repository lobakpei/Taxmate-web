...
Run `node scripts/preview-server.js`, then open `http://127.0.0.1:4173/?auth-restored=rc12` without invoking Google OAuth. Exact final commit/tree and regression totals are supplied in the Founder handoff.

At candidate freeze, no RC.12 production code deployment, push or merge had occurred. Matching LIVE Price objects were created only after Sandbox validation; no real customer, card, payment or subscription was used. Founder has authorised a controlled release of the exact frozen candidate, followed by immediate production verification.
