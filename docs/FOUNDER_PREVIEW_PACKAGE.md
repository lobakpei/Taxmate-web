...
Candidate branch is `codex/taxmate-modernisation-20260817`. Runtime identity is app `2.0.0-rc.1`, build `2026-08-19.stripe-sandbox-rc.6`, state/sync schema 5, tax rules `2026-27.2026-08-17.1`, form map `2025-26.2026-04-06.1`, PWA cache `taxmate-v2-rc-1-stripe-sandbox-rc-6`.
...
Run `node scripts/preview-server.js`, then open `http://127.0.0.1:4173/?stripe-rc=6`. The integrated repository gate is 106/106 and the deployed staging browser audit is 29/29. Correct-account external Stripe hosted receipts and lifecycle gates pass. Existing Firebase configuration, the persistent `taxmate-staging` target and approved GA4/Sentry receipt checks are documented in `STAGING_EXTERNAL_SERVICE_REPORT.md`. No production merge, production rule/Functions deployment, Stripe LIVE configuration, migration or production deployment occurred.
...
Correct-account Stripe Sandbox products, prices, promotions, hosted Checkout and lifecycle now pass. Production remains blocked by live ICO correspondence-address propagation and the Founder-controlled Blaze billing link required to finish staging Functions/Storage, Google/App Check, persistent Stripe webhook/callable Terms-profile and real receipt validation. See `PRODUCTION_READINESS_REPORT.md`, `STRIPE_SANDBOX_REPORT.md` and `KNOWN_LIMITATIONS.md`.
