...
Candidate branch is `codex/taxmate-modernisation-20260817`. Runtime identity is app `2.0.0-rc.1`, build `2026-08-19.external-services-rc.2`, state/sync schema 5, tax rules `2026-27.2026-08-17.1`, form map `2025-26.2026-04-06.1`, PWA cache `taxmate-v2-rc-1-external-services-rc-2`.
...
Run `node scripts/preview-server.js`, then open `http://127.0.0.1:4173/?external-services-rc=2`. The integrated repository gate is 94/94 and the separate real Stripe TEST integration is 1/1. Existing Firebase configuration and the TaxMate Stripe sandbox are documented in `STAGING_EXTERNAL_SERVICE_REPORT.md`. No production merge, rule/Functions deployment, Stripe LIVE configuration, migration or production deployment occurred.
...
Production remains blocked pending live ICO propagation, the verified correspondence address, Founder/legal/VAT/refund decisions, candidate Firebase staging deployment, hosted TEST checkout approval and telemetry provider access. See `LEGAL_PRIVACY_RELEASE_GATE_REPORT.md` and `KNOWN_LIMITATIONS.md`.
