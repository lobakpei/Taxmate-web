...
Candidate branch is `codex/taxmate-modernisation-20260817`. Runtime identity is app `2.0.0-rc.1`, build `2026-08-19.legal-privacy-gate.2`, state/sync schema 5, tax rules `2026-27.2026-08-17.1`, form map `2025-26.2026-04-06.1`, PWA cache `taxmate-v2-rc-1-legal-privacy-gate-2`. The Legal Gate changes remain uncommitted at this stop unless the handoff explicitly reports otherwise.
...
Run `node scripts/preview-server.js`, then open `http://127.0.0.1:4173/`. The integrated local gate is 82/82 and the final browser matrix, including the legal/runtime addendum, passed. External staging remains a release blocker as detailed in `STAGING_EXTERNAL_SERVICE_REPORT.md`. No production merge, rule/Functions deployment, Stripe configuration, migration or production deployment occurred.
