...
Read-only external verification on 19 August 2026 confirmed existing project `taxmate-uk-2`, Web app `taxmate-web`, Hosting, Native Firestore, Storage, Google provider discovery and a Web App Check registration. Firestore and Storage App Check are enforced; Auth is unenforced. The Cloud Functions API is disabled/not previously used, so the candidate callable and webhook definitions are not deployed. Production was not enabled or used as staging. Deployed Google Sign-In, candidate Functions/App Check and real cloud receipt/full-ZIP behavior must still be repeated in approved non-production isolation before release.
