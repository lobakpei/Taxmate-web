...
class TaxMateLtdUIFacade{
  ...
  onResumeCompanyDraft(input){const saved=this.drafts.getSetup();return this.execute('onResumeCompanyDraft',input,function(){
    const result=this.resumeDraft(),rank=route=>{const match=/^ltd\.onboarding\.step([1-5])$/.exec(route||'');return match?Number(match[1]):route==='ltd.onboarding.registration-details'?1:null;};
    const canonicalRank=rank(result.nextRoute),savedRank=rank(saved&&saved.resumeScreen);
    // A saved visual position may resume at the same or an earlier setup page,
    // but it must never jump over a newly required canonical prerequisite.
    if(result.status==='ok'&&saved?.companyId===this.activeProfile()?.entityId&&savedRank!=null&&canonicalRank!=null&&savedRank<=canonicalRank)result.nextRoute=saved.resumeScreen;
    return result;
  });}
  ...
  onLookupCompaniesHouse(input){
    if(typeof this.driver.lookupChangesSetupIdentity==='function'&&this.driver.lookupChangesSetupIdentity(input&&input.companyNumber))this.drafts.patchField('ltd.onboarding.step1',{id:'identityDetailsConfirmed',type:'select-one',value:''});
    return this.execute('onLookupCompaniesHouse',input,this.driver.lookupCompany);
  }
  ...
  onDraftChanged(input={}){if(!input.screenId||!input.field)return Promise.resolve(this.result({status:'field_error',fieldErrors:[fieldError('draft','screen_and_field_required')]}));this.drafts.patchField(input.screenId,input.field);if(input.screenId==='ltd.onboarding.step1'&&input.field.id==='identityDetailsConfirmed'&&input.field.value!=='yes'&&input.field.value!==true&&typeof this.driver.clearSetupIdentityConfirmation==='function')this.driver.clearSetupIdentityConfirmation();this.emit();return Promise.resolve(this.result({status:'ok',data:{draft:this.drafts.get(input.screenId)}}));}
