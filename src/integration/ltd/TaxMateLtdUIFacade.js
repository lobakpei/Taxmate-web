...
class TaxMateLtdUIFacade{
  ...
  onBack(){const routes=this.workflow.snapshot().routes;if(this.inCompanySetup()&&!this.workflow.topOverlay()){const step=/^ltd\.onboarding\.step([1-5])$/.exec(routes.at(-1)?.screenId||'');if(step&&Number(step[1])>1){const previous='ltd.onboarding.step'+(Number(step[1])-1);if(routes.at(-2)?.screenId===previous)this.workflow.back();else this.workflow.enter(previous,{}, {replace:true});this.emit();return Promise.resolve(this.result({status:'ok',nextRoute:previous}));}if(step||!/^ltd\.onboarding\./.test(routes.at(-2)?.screenId||''))return this.onDismissRequested({reason:'back'});}const outcome=this.workflow.back();this.emit();return Promise.resolve(this.result({status:'ok',data:{outcome},nextRoute:this.workflow.currentRoute()&&this.workflow.currentRoute().screenId||null}));}
