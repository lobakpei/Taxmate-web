(function(root,factory){
  ...
  const api=factory(
    ...
    node?require('../../core/company-books'):root.TaxMateCompanyBooks,
    ...
    node?require('../../core/ltd-sync'):root.TaxMateLtdSync
  );
  if(node)module.exports=api;
  root.TaxMateCanonicalCompanyDriver=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(CompanyAccess,CompanyBooks,CompanyIdentity,CompanyLedger,CompanyProfile,CompanyProfileHistory,CompanyRemuneration,CompanyRemunerationRules,CompanyScenario,CompanyTax,CompanyTreatment,CompanyWorkspace,Domain,Entitlement,Money,Partnership,State,TransactionAdapter,Repository,CompaniesHouse,LtdSync){
'use strict';
...
const COPY_KEY_BY_REASON=Object.freeze({
  ...
  founder_name_required:'error.required',founder_shares_invalid:'error.ownership_total',other_shares_invalid:'error.ownership_total',share_ownership_total_required:'error.ownership_total',other_shareholder_name_required:'error.other_name',ownership_total_invalid:'error.ownership_total',
...
function historyFacts(profile){return [...(profile.statutoryReviewHistory||[]),...(profile.statutoryReview?[profile.statutoryReview]:[])].map(r=>r.facts).concat(profile.retentionVatCrossings?[profile.retentionVatCrossings]:[]);}
...
class CanonicalCompanyDriver{
  constructor(options={}){
    ...
    this.pendingCompanyCreation=null;
    ...
    this.manageCompanySetup=typeof options.manageCompanySetup==='function'?options.manageCompanySetup:null;
    this.setupProtocol=null;
    ...
    this.lookup=persistedVerification?{status:['verified','needs_checking','manual_unverified','not_registered'].includes(persistedVerification.status)?'found':persistedVerification.status,verificationStatus:persistedVerification.status,number:persistedVerification.companyNumber||null,founderShortcut:false,company:{number:persistedVerification.companyNumber||null,name:persistedVerification.registryFacts.legalName||null,incorporationDate:persistedVerification.registryFacts.incorporationDate||null,status:persistedVerification.registryFacts.companyStatus||null,type:persistedVerification.registryFacts.companyType||null,registryUrl:persistedVerification.registryFacts.registryUrl||null},reasons:clone(persistedVerification.reasonCodes),retryable:persistedVerification.retryable===true}: {status:'idle',number:null,company:null,reasons:[]};
  ...
  persist(){this.validate();if(this.pendingCompanyCreation&&!this.pendingCompanyCreation.claimed)return true;this.repository.replace(this.state);return true;}
  ...
  reload(){if(this.pendingCompanyCreation&&!this.pendingCompanyCreation.claimed)return this.readSnapshot();this.state=this.repository.load();State.validateState(this.state);const persisted=this.activeProfile()&&this.activeProfile().registryVerification;if(persisted)this.lookup={status:['verified','needs_checking','manual_unverified','not_registered'].includes(persisted.status)?'found':persisted.status,verificationStatus:persisted.status,number:persisted.companyNumber||null,founderShortcut:false,company:{number:persisted.companyNumber||null,name:persisted.registryFacts&&persisted.registryFacts.legalName||null,incorporationDate:persisted.registryFacts&&persisted.registryFacts.incorporationDate||null,status:persisted.registryFacts&&persisted.registryFacts.companyStatus||null,type:persisted.registryFacts&&persisted.registryFacts.companyType||null,registryUrl:persisted.registryFacts&&persisted.registryFacts.registryUrl||null},reasons:clone(persisted.reasonCodes||[]),retryable:persisted.retryable===true};this.companyDraftState=this.companyDraftFromProfile(this.activeProfile());return this.readSnapshot();}
  ...
  effectiveShareholders(profile=this.activeProfile()){if(!profile)return[];try{return CompanyProfileHistory.ownershipAtDate(profile,this.currentDate()).shareholders;}catch(_){const verified=CompanyProfileHistory.verifiedOwnershipAtDate(profile,this.currentDate());return verified?verified.shareholders:[];}}
  ...
  routeAccess(action,nextRoute,data={}){this.requireAccess(action);return{status:'ok',data:{...clone(data),noWrite:true},routeParams:clone(data),nextRoute};}
  ...
  updatePayrollReporting(input={}){
    this.requireAccess('confirm_salary');
    const record=this.recordsFor('salaryRecords').find(r=>r.id===input.recordId);
    if(!record)return{status:'field_error',fieldErrors:[fieldError('recordId','record_not_found')]};
    const current=record.payrollReporting,history=record.payrollReportingHistory||[];
    const payload={requestId:input.requestId,sourceEventRevisionId:input.sourceEventRevisionId,status:input.status,reportedOn:input.reportedOn,evidenceRefs:clone(input.evidenceRefs||[]),reason:input.reason};
    const previous=[...history,...(current?[current]:[])].find(r=>r.requestId===input.requestId);
    if(previous){
      const matches=Object.entries(payload).every(([k,v])=>JSON.stringify(previous[k])===JSON.stringify(v));
      return{status:matches?'ok':'review_required',reviewReasons:matches?[]:['payroll_reporting_request_conflict'],data:matches?{record:clone(record),idempotent:true}:null};
    }
    if(input.sourceEventRevisionId!==record.sourceEventRevisionId||input.expectedReportingRevision!==(current?.revision||0))return{status:'review_required',reviewReasons:['payroll_reporting_source_changed']};
    try{
      if(input.reportedOn>this.currentDate())throw new Error('Future reporting date');
      const updatedAt=Math.max(this.now(),record.updatedAt+1,(current?.updatedAt||0)+1);
      const reporting={...payload,schemaVersion:1,revision:(current?.revision||0)+1,updatedAt,deviceId:this.deviceId};
      // RTI evidence is not another economic event or a financial revision.
      const next={...clone(record),payrollReporting:reporting,payrollReportingHistory:[...clone(history),...(current?[clone(current)]:[])],updatedAt,deviceId:this.deviceId};
      Domain.validateSalaryRecord(next);this.atomic(()=>this.upsert('salaryRecords',next));
      return{status:'ok',data:{record:clone(next),idempotent:false},nextRoute:'ltd.tax.salary-record'};
    }catch(_){return{status:'field_error',fieldErrors:[fieldError('payrollReporting','payroll_reporting_evidence_required')]};}
  }
  saveBookResult(name,result){return this.atomic(()=>{const envelopes=Array.isArray(result.envelopes)?result.envelopes:result.envelope?[result.envelope]:[];for(const envelope of envelopes)this.upsert('economicEvents',envelope);if(result.record)this.upsert(name,result.record);return result;});}
  ...
  companyDraftFromProfile(profile){
    ...
    profile=CompanyProfile.normalize(profile);const setup=CompanyProfile.setupAnswersFor(profile),directorAnswer=setup.directorAnswer||'not_sure';
    return{
      status:profile.lifecycleStatus==='confirmed'?'confirmed':setup.registrationAnswer!=='yes'?'registration_pending':setup.tradingAnswer==='not_sure'?'trading_review_required':setup.soleShareholderAnswer==='not_sure'?'ownership_review_required':'in_progress',
      registrationStatus:setup.registrationAnswer==='yes'?'registered':setup.registrationAnswer==='not_sure'?'needs_checking':'not_available',directorAnswer,soleShareholderAnswer:setup.soleShareholderAnswer||null,
  ...
  companyYearFigures(profile=this.activeProfile()){
    if(!profile||profile.lifecycleStatus!=='confirmed'||!profile.accountingPeriod)return null;
    return CompanyBooks.buildCompanyYearFigures({profile,events:this.eventsFor(profile),periodRecords:this.recordsFor('companyTaxPeriods',profile),salesInvoices:this.recordsFor('salesInvoices',profile),supplierBills:this.recordsFor('supplierBills',profile),fixedAssets:this.recordsFor('fixedAssets',profile),bankReconciliations:this.recordsFor('bankReconciliations',profile),asOfDate:this.currentDate()});
  }

  statutoryInput(profile=this.activeProfile(),filingFacts={}){
    return{profile,figures:this.companyYearFigures(profile),events:this.eventsFor(profile),periodRecords:this.recordsFor('companyTaxPeriods',profile),salaryRecords:this.recordsFor('salaryRecords',profile),dividendDeclarations:this.recordsFor('dividendDeclarations',profile),salesInvoices:this.recordsFor('salesInvoices',profile),supplierBills:this.recordsFor('supplierBills',profile),fixedAssets:this.recordsFor('fixedAssets',profile),bankReconciliations:this.recordsFor('bankReconciliations',profile),filingFacts:clone(filingFacts),vatHistoryIncomplete:!!(this.state.retention?.historyIncomplete||profile?.retentionHistoryGap),deadlines:this.projection(profile)?.deadlines||null,asOfDate:this.currentDate(),generatedAt:this.now()};
  }
  statutorySnapshot(profile=this.activeProfile()){
    if(!profile||profile.lifecycleStatus!=='confirmed')return null;
    try{const pack=CompanyBooks.buildSelfFilingPack(this.statutoryInput(profile));return{checklist:pack.statutoryChecklist,readiness:pack.readiness};}catch(_){return{status:'needs_checking',reasonCode:'statutory_checklist_needs_checking',readiness:{figuresReady:false,statutoryObligationsComplete:false,officialSubmissionVerified:false}};}
  }
  saveStatutoryReview(input={}){
    this.requireAccess('edit_company');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};
    try{
      const context=this.statutoryInput(profile),fingerprint=CompanyBooks.statutoryFingerprint(context),prior=profile.statutoryReview;
      if(input.sourceFingerprint!==fingerprint||input.expectedRevision!==(prior?.revision||0))return{status:'review_required',reviewReasons:['statutory_review_source_changed'],data:{sourceFingerprint:fingerprint,reviewRevision:prior?.revision||0}};
      const review={schemaVersion:1,id:'statutory-review:'+profile.entityId,entityId:profile.entityId,startDate:profile.accountingPeriod.startDate,endDate:profile.accountingPeriod.endDate,reviewedOn:this.currentDate(),sourceFingerprint:fingerprint,revision:(prior?.revision||0)+1,updatedAt:this.now(),deviceId:this.deviceId,facts:clone(input.facts||{})};
      CompanyBooks.validateStatutoryReview(review);
      const vat=CompanyBooks.vatReview({...context,asOfDate:this.currentDate()},review.facts,historyFacts(profile));
      const crossing=vat.crossings.filter(c=>c.kind==='rolling_12_months').map(c=>c.monthEnd).sort()[0];
      if(crossing)review.facts.vatFirstExceededMonthEnd={value:crossing,evidenceRefs:['statutory-source:'+fingerprint,...vat.sourceRecords.map(r=>r.recordId+':'+r.revision)]};
      const future=vat.crossings.filter(c=>c.kind==='future_30_days').map(c=>c.knownOn).sort()[0];
      if(future)review.facts.vatUnresolvedFutureKnownOn={value:future,evidenceRefs:clone(review.facts.vatExpectedKnownOn?.evidenceRefs||['statutory-source:'+fingerprint])};
      CompanyBooks.validateStatutoryReview(review);
      const history=[...(profile.statutoryReviewHistory||[]),...(prior?[clone(prior)]:[])];
      this.saveProfile({...clone(profile),statutoryReview:review,statutoryReviewHistory:history,updatedAt:this.now(),deviceId:this.deviceId});
      return{status:'ok',data:{review:clone(review),statutory:this.statutorySnapshot()},nextRoute:'ltd.tax.company-year'};
    }catch(error){return{status:'field_error',fieldErrors:[fieldError('statutoryFacts',stableReason(error,'statutory_facts_need_checking'))]};}
  }
  ...
  companyBusinessRow(profile){
    const effectiveShareholders=this.effectiveShareholders(profile),entity=this.entityFor(profile),ledger=CompanyLedger.reconcile(this.eventsFor(profile),profile.entityId),projection=profile.lifecycleStatus==='confirmed'?this.projection(profile):null,drafts=this.eventsFor(profile).filter(event=>event.status==='draft'),reviewReasons=projection?projection.reviewItems.map(item=>item.reasonCode):[],access=this.access(profile.lifecycleStatus==='confirmed'?'read':'resume_company_draft');
    return{
      ...
      share:{basis:'issued_shares',percent:effectiveShareholders.find(item=>item.isAccountHolder)?.ownershipBasisPoints/100||null,applied:true,status:effectiveShareholders.length?'confirmed':'draft'},
      summary:{kind:'company_accounting_profit',amountMinor:profile.retentionHistoryGap?.requiresReview?null:ledger.accountingProfitMinor,companyCashMinor:ledger.cashMinor,status:profile.retentionHistoryGap?.requiresReview?'review_required':profile.lifecycleStatus==='confirmed'?'supported':'draft',reasonCodes:profile.retentionHistoryGap?.requiresReview?['retention_history_incomplete']:profile.lifecycleStatus==='confirmed'?[]:['company_setup_incomplete']},
  ...
  readSnapshot(){
    const existing=this.activeProfile(),read=existing?this.access('read'):null;
    if(existing&&read&&!read.allowed){
      const resolved=Entitlement.resolve(this.entitlementSnapshot,this.now(),false);
      return{
        contractVersion:'taxmate-ltd-ui-facade.3',packageStatus:'INTEGRATED_CANDIDATE',mode:this.mode,dataset:null,
        context:{taxYear:this.taxYearRange().taxYear,currentDate:this.currentDate()},
        businessList:this.state.businesses.map(b=>this.legacyBusinessRow(b)),
        companyLimit:{maximum:1,activeCount:1,activeCompanyId:this.trustedActiveCompanyId||existing.entityId,canCreate:false,additionalLtdSupported:false,requiredTier:'pro',reason:read.reason,existingAction:null},
        company:null,lookupStatus:{status:'unavailable'},statutory:null,
        workspace:{projection:null,projectionError:null,companyYearFigures:null,companyYearError:null,events:[],salesInvoices:[],supplierBills:[],fixedAssets:[],bankReconciliations:[],salaryRecords:[],dividendDeclarations:[],ownershipHistory:[]},
        retention:clone(read.retention),
        ...
        informationCopy:clone(this.copy),runtime:clone(this.runtime)
      };
    }
    ...
    let projection=null,projectionError=null,companyYearFigures=null,companyYearError=null;const periodPlan=this.periodPlan(profile);
    ...
    try{companyYearFigures=this.companyYearFigures(profile);}catch(_){companyYearError={reasonCode:'company_year_figures_need_checking',copyKey:'error.fix_issue',params:{}};}
    const resolved=Entitlement.resolve(this.entitlementSnapshot,this.now(),false);
    ...
    return{
      ...
      company:profile?{entity:clone(entity),profile:clone(profile),setupAnswers:clone(CompanyProfile.setupAnswersFor(profile)),draftState:clone(this.companyDraftState),bookkeepingEligibility:clone(gate),taxEstimateEligibility:clone(taxGate),periodPlan:clone(this.pendingPeriodPlan||periodPlan)}:null,
      lookupStatus:clone(this.lookup),
      // Retained-access facts for the UI (same canonical decision as the blocked branch;
      // null for active Pro). Presentation only — the guard remains requireAccess().
      retention:clone(read&&read.retention||null),
      statutory:readAccess.allowed?this.statutorySnapshot(profile):null,
      workspace:{projection:clone(projection),projectionError,companyYearFigures:clone(companyYearFigures),companyYearError,events:clone(this.eventsFor(profile)),salesInvoices:clone(this.recordsFor('salesInvoices',profile)),supplierBills:clone(this.recordsFor('supplierBills',profile)),fixedAssets:clone(this.recordsFor('fixedAssets',profile)),bankReconciliations:clone(this.recordsFor('bankReconciliations',profile)),salaryRecords:clone(this.recordsFor('salaryRecords',profile)),dividendDeclarations:clone(this.recordsFor('dividendDeclarations',profile)),ownershipHistory:clone(profile&&Array.isArray(profile.shareholders)&&profile.shareholders.length?CompanyProfileHistory.ensureHistory(profile).ownershipHistory:[])},
      entitlement:{...resolved,planMappingStatus:'FOUNDER_APPROVED_PRO_ONLY',commercialGateApplied:this.enforceEntitlement,planMapping:clone(this.planMapping),actions:Object.fromEntries(Array.from(CompanyAccess.LTD_PRO_ACTIONS).map(action=>[action,this.access(action)]))},
  ...
  async chooseBusinessCategory(input={}){
    ...
    const now=this.now(),requestedId=cleanText(input.companyId,128)||null,entityId=this.trustedActiveCompanyId||requestedId||this.newId('company').slice(0,110)+':'+Math.random().toString(36).slice(2,10);
    ...
    return{status:'ok',data:{limitReached:false,profile:clone(profile),activeCompanyClaim:{pending:true,noWrite:true}},nextRoute:'ltd.onboarding.step1'};
  }
  ...
  resumeRoute(profile){if(!profile)return'ltd.onboarding.step1';profile=CompanyProfile.normalize(profile);const setup=CompanyProfile.setupAnswersFor(profile);if(setup.registrationAnswer!=='yes'||setup.identityDetailsConfirmed!==true)return'ltd.onboarding.registration-details';if(setup.tradingAnswer==='not_sure')return'ltd.onboarding.step2';if(setup.soleShareholderAnswer==='not_sure')return'ltd.onboarding.step3';const next=CompanyProfile.missingQuestion(profile),step=next&&['legal_name','company_type','company_number','incorporation_date','registration_resolution','identity_confirmation'].includes(next.id)?1:next&&['trading_status','trading_resolution','accounting_period','corporation_tax_status'].includes(next.id)?2:next&&['account_holder_roles','share_structure','ownership_resolution'].includes(next.id)?3:next&&['activity_profile','unsupported_screen'].includes(next.id)?4:5;return`ltd.onboarding.step${step}`;}
  ...
  clearSetupIdentityConfirmation(options={}){
    const profile=this.activeProfile();
    if(!profile||profile.lifecycleStatus==='confirmed')return false;
    const setup=CompanyProfile.setupAnswersFor(profile);
    const resetDownstream=options.resetDownstream===true;
    if(!resetDownstream&&setup.identityDetailsConfirmed===false&&profile.setupAnswers&&profile.setupAnswers.identityDetailsConfirmed===false)return false;
    let next=clone(profile);
    if(resetDownstream){for(const field of ['tradingStatus','tradingStartDate','accountingReferenceDate','accountingPeriod','corporationTaxStatus','accountHolder','activityType','confirmedAt'])delete next[field];next.directors=[];next.shareholders=[];next.shareClasses=[];next.riskAnswers={};this.pendingPeriodPlan=null;}
    next=CompanyProfile.setSetupAnswers(next,{identityDetailsConfirmed:false,...(resetDownstream?{tradingAnswer:null,directorAnswer:null,soleShareholderAnswer:null}:{})},{now:this.now(),deviceId:this.deviceId});
    this.saveProfile(next);this.companyDraftState=this.companyDraftFromProfile(next);return true;
  }
  lookupTargetsDifferentCompany(companyNumber){
    const profile=this.activeProfile();
    if(!profile)return false;
    const number=CompanyIdentity.normalizeCompanyNumber(companyNumber||''),shortcut=typeof this.companiesHouseProvider.acceptsAlias==='function'&&this.companiesHouseProvider.acceptsAlias(number),validation=shortcut?{valid:true,normalized:number.toLowerCase()}:CompanyIdentity.validateCompanyNumber(number);
    if(!validation.valid)return false;
    const previousShortcut=profile.companyNumberStatus==='not_available'&&Domain.isoDate(profile.incorporationDate)&&profile.registryVerification&&profile.registryVerification.provider==='user_fact';
    const previous=previousShortcut?'lobakpe1':CompanyIdentity.normalizeCompanyNumber(profile.companyNumber||'');
    return (validation.normalized||number)!==previous;
  }
  lookupChangesSetupIdentity(companyNumber){const profile=this.activeProfile();return !!(profile&&profile.lifecycleStatus!=='confirmed'&&CompanyProfile.setupAnswersFor(profile).identityDetailsConfirmed===true&&this.lookupTargetsDifferentCompany(companyNumber));}
  pendingSetup(){const pending=this.pendingCompanyCreation,profile=this.activeProfile();if(!pending||pending.claimed||!profile)return null;return{schemaVersion:1,entityId:pending.entityId,claimAttempted:pending.claimAttempted===true,profile:clone(profile),entity:clone(this.entityFor(profile)),lookup:clone(this.lookup)};}
  restorePendingSetup(saved){
    if(!saved||saved.schemaVersion!==1||!saved.profile||saved.profile.lifecycleStatus!=='draft'||saved.profile.deletedAt!=null||saved.entityId!==saved.profile.entityId||saved.profile.id!=='company-profile:'+saved.entityId||saved.entity?.id!==saved.entityId||saved.entity.type!=='limited_company'||saved.entity.deletedAt!=null)return false;
    const latest=this.repository.load();if((latest.domain.companyProfiles||[]).some(p=>p.deletedAt==null||p.entityId===saved.entityId)||this.trustedActiveCompanyId&&this.trustedActiveCompanyId!==saved.entityId||(latest.domain.entities||[]).some(e=>e.id===saved.entityId))return false;
    const candidate=clone(latest);candidate.domain.companyProfiles.push(clone(saved.profile));candidate.domain.entities.push(clone(saved.entity));State.validateState(candidate);
    this.state=candidate;this.pendingCompanyCreation={entityId:saved.entityId,claimed:false,claimAttempted:saved.claimAttempted===true,baseState:clone(latest)};this.lookup=clone(saved.lookup||{status:'idle',reasons:[]});this.companyDraftState=this.companyDraftFromProfile(saved.profile);return true;
  }
  async inspectSetupExit(input={}){
    this.requireAccess('resume_company_draft');
    if(input.companyId&&this.manageCompanySetup)return this.manageCompanySetup({action:'inspect',companyId:input.companyId});
    const profile=this.activeProfile();if(!profile||profile.lifecycleStatus==='confirmed')return{status:'completed',canDiscard:false,reason:'setup_already_completed'};
    if(this.pendingSetup()&&!this.pendingCompanyCreation.claimAttempted&&!this.trustedActiveCompanyId)return{status:'local',canDiscard:true,companyId:profile.entityId};
    if(!this.manageCompanySetup)return{status:'legacy',canDiscard:false,reason:'setup_legacy_slot',companyId:profile.entityId};
    return this.manageCompanySetup({action:'inspect',companyId:profile.entityId});
  }
  async discardCompanySetup(input={}){
    this.requireAccess('resume_company_draft');const profile=this.activeProfile(),companyId=input.companyId;if(!companyId||profile?.entityId===companyId&&profile.lifecycleStatus==='confirmed'||profile?.entityId!==companyId&&input.recovery!==true)throw Object.assign(new Error('setup_not_unfinished'),{code:'setup_not_unfinished'});
    if(this.pendingSetup()&&!this.pendingCompanyCreation.claimAttempted&&!this.trustedActiveCompanyId){this.state=this.repository.load();this.pendingCompanyCreation=null;this.lookup={status:'idle',reasons:[]};this.companyDraftState=this.companyDraftFromProfile(null);return{status:'ok',data:{discardedEntityId:profile.entityId,localOnly:true},nextRoute:'home'};}
    if(!this.manageCompanySetup)throw Object.assign(new Error('setup_legacy_slot'),{code:'setup_legacy_slot'});
    const result=await this.manageCompanySetup({action:'discard',companyId,expectedVersion:input.expectedVersion,operationId:input.operationId});
    if(result.status!=='discarded'||result.companyId!==companyId||!Array.isArray(result.records))throw Object.assign(new Error('setup_discard_not_confirmed'),{code:'setup_discard_not_confirmed'});
    const latest=this.repository.load();this.state=LtdSync.applyDownloads(latest,result.records);this.pendingCompanyCreation=null;if(this.trustedActiveCompanyId===companyId)this.trustedActiveCompanyId=null;this.lookup={status:'idle',reasons:[]};this.companyDraftState=this.companyDraftFromProfile(this.activeProfile());this.persist();
    return{status:'ok',data:{discardedEntityId:companyId,localOnly:false},nextRoute:'home'};
  }
  fixCompanyFact(input={}){
    this.requireAccess('resume_company_draft');
    const reason=String(input.reasonCode||''),step=/(registration|legal_name|company_name|company_number|incorporation)/.test(reason)?1:/(trading|accounting|period|corporation_tax_status)/.test(reason)?2:/(director|shareholder|ownership|share_structure|account_holder)/.test(reason)?3:/(activity|ordinary|service|digital|group|associated|property|inventory|vat|unsupported)/.test(reason)?4:null;
  ...
  async lookupCompany(input={}){
    ...
    const shortcutAlias=typeof this.companiesHouseProvider.acceptsAlias==='function'&&this.companiesHouseProvider.acceptsAlias(number),validation=shortcutAlias?{valid:true,normalized:number.toLowerCase()}:CompanyIdentity.validateCompanyNumber(number);
    ...
    const profileBeforeLookup=this.activeProfile();
    if(profileBeforeLookup&&profileBeforeLookup.lifecycleStatus==='confirmed'&&this.lookupTargetsDifferentCompany(validation.normalized||number))return{status:'review_required',reviewReasons:['identity_change_requires_company_edit'],data:{companyNumber:validation.normalized||number,noWrite:true},nextRoute:'ltd.records.company-edit'};
    if(this.lookupChangesSetupIdentity(validation.normalized||number))this.clearSetupIdentityConfirmation({resetDownstream:true});
    ...
    const shortcut=result&&result.founderShortcut===true,resultNumber=result&&result.company&&CompanyIdentity.validateCompanyNumber(result.company.number||''),resolvedNumber=resultNumber&&resultNumber.valid?resultNumber.normalized:(shortcut?null:(validation.normalized||number)),reasons=Array.from(new Set([...(result.reasonCodes||[]),...(result.reasonCode?[result.reasonCode]:[])]));
    this.lookup={status:result.status,verificationStatus:result.status==='found'?(result.verificationStatus||CompaniesHouse.assessRegistryCompany(result.company).verificationStatus):result.status,number:resolvedNumber,inputAlias:shortcut?number.toLowerCase():null,founderShortcut:shortcut,company:clone(result.company||null),reasons,retryable:result.retryable===true};
    const profile=this.activeProfile();
    if(profile){
      ...
      if(shortcut)return{status:'ok',data:clone(this.lookup),nextRoute:null};
      const hasConfirmedManualFacts=profile.companyNumberStatus==='provided'&&profile.companyNumber===resolvedNumber;
      ...
      const provider='companies_house_api',verification={schemaVersion:1,status:storedStatus,companyNumber:resolvedNumber,checkedAt:this.now(),verifiedAt:storedStatus==='verified'?this.now():profile.registryVerification&&profile.registryVerification.verifiedAt||null,verificationSource:provider,provider,retryable:result.retryable===true,reasonCodes:storedReasons,registryFacts:{legalName:company.name||null,incorporationDate:company.incorporationDate||null,companyStatus:company.status||null,companyType:company.type||null,registryUrl:company.registryUrl||null},userFacts:{legalName:profile.legalName||null,incorporationDate:profile.incorporationDate||null}},candidate={...profile,registryVerification:verification,updatedAt:this.now(),deviceId:this.deviceId},assessment=CompanyProfile.assess(candidate);
  ...
  async continueStep(input={}){
    ...
    try{
      if(step===1){
        const previousSetup=CompanyProfile.setupAnswersFor(profile);
        const shortcutInput=String(values.companyNumber||'').trim().toLowerCase();
        if(shortcutInput==='lobakpe1'&&(!this.lookup||this.lookup.founderShortcut!==true)){
          const restoredLookup=await this.lookupCompany({companyNumber:shortcutInput});
          if(restoredLookup.status!=='ok')return restoredLookup;
        }
        const registrationAnswer=CompanyProfile.SETUP_ANSWER_VALUES.includes(values.registrationAnswer)?values.registrationAnswer:values.companyNumberStatus==='provided'?'yes':values.companyNumberStatus==='not_available'?'no':null;
        const numberStatus=registrationAnswer==='yes'?'provided':registrationAnswer==='no'?'not_available':undefined,shortcutFromLookup=this.lookup&&this.lookup.founderShortcut===true&&String(values.companyNumber||'').trim().toLowerCase()==='lobakpe1',shortcutFromSaved=registrationAnswer==='yes'&&!String(values.companyNumber||'').trim()&&profile.companyNumberStatus==='not_available'&&Domain.isoDate(profile.incorporationDate)&&profile.registryVerification&&profile.registryVerification.provider==='user_fact',founderShortcut=shortcutFromLookup||shortcutFromSaved,storedNumberStatus=founderShortcut?'not_available':numberStatus;let number=null;if(!CompanyProfile.SETUP_ANSWER_VALUES.includes(registrationAnswer))errors.push(fieldError('companyNumberStatus','answer_required'));
        const name=CompanyIdentity.validateCompanyName(values.legalName||'',{requirePrivateEnding:registrationAnswer==='yes'});if(!name.valid)errors.push(fieldError('legalName',name.reason,{invalidCharacters:name.invalidCharacters||[]}));
        const date=values.incorporationDate;
        if(registrationAnswer==='yes'){
          if(!founderShortcut){const valid=CompanyIdentity.validateCompanyNumber(values.companyNumber||'');if(!valid.valid)errors.push(fieldError('companyNumber',valid.reason));else number=valid.normalized;}
          ...
          if(values.identityDetailsConfirmed!==true&&values.identityConfirmed!==true)errors.push(fieldError('identityDetailsConfirmed','confirmation_required'));
        }
        if(errors.length)return{status:'field_error',fieldErrors:errors};
        const previousShortcut=profile.companyNumberStatus==='not_available'&&Domain.isoDate(profile.incorporationDate)&&profile.registryVerification&&profile.registryVerification.provider==='user_fact';
        const previousIdentity=[previousSetup.registrationAnswer||'',previousShortcut?'founder-shortcut':profile.companyNumber||'',profile.legalName||'',profile.incorporationDate||''].join('|');
        const nextIdentity=[registrationAnswer,founderShortcut?'founder-shortcut':number||'',name.name||'',registrationAnswer==='yes'?date||'':''].join('|');
        const resetDownstream=registrationAnswer!=='yes'||previousIdentity!==nextIdentity;
        ...
        next=CompanyProfile.answer(next,'company_number',{status:storedNumberStatus,number},{now,deviceId:this.deviceId});
        if(registrationAnswer==='yes')next=CompanyProfile.answer(next,'incorporation_date',date,{now,deviceId:this.deviceId});
        else {next=clone(next);delete next.incorporationDate;}
        if(resetDownstream){next=clone(next);for(const field of ['tradingStatus','tradingStartDate','accountingReferenceDate','accountingPeriod','corporationTaxStatus','accountHolder','activityType','confirmedAt'])delete next[field];next.directors=[];next.shareholders=[];next.shareClasses=[];next.riskAnswers={};this.pendingPeriodPlan=null;}
        if(founderShortcut){
          next.registryVerification={schemaVersion:1,status:'not_registered',companyNumber:null,checkedAt:now,verifiedAt:null,verificationSource:'user_fact',provider:'user_fact',retryable:false,reasonCodes:['company_not_yet_registered'],registryFacts:{legalName:name.name,incorporationDate:date,companyStatus:null,companyType:null,registryUrl:null},userFacts:{legalName:name.name,incorporationDate:date}};
        }else if(registrationAnswer==='yes'){
        ...
        }else if(registrationAnswer==='no')next.registryVerification={schemaVersion:1,status:'not_registered',companyNumber:null,checkedAt:now,provider:'user_fact',retryable:false,reasonCodes:['company_not_yet_registered'],registryFacts:{legalName:null,incorporationDate:null,companyStatus:null,companyType:null,registryUrl:null}};
        else next.registryVerification={schemaVersion:1,status:'not_checked',companyNumber:null,checkedAt:null,provider:'user_fact',retryable:false,reasonCodes:['company_registration_needs_checking'],registryFacts:{legalName:null,incorporationDate:null,companyStatus:null,companyType:null,registryUrl:null}};
        next=CompanyProfile.setSetupAnswers(next,{registrationAnswer,identityDetailsConfirmed:registrationAnswer==='yes'?true:null,...(resetDownstream?{tradingAnswer:null,directorAnswer:null,soleShareholderAnswer:null}:{})}, {now,deviceId:this.deviceId});
        this.companyDraftState={...this.companyDraftState,status:registrationAnswer==='yes'?'in_progress':'registration_pending',registrationStatus:registrationAnswer==='yes'?'registered':registrationAnswer==='not_sure'?'needs_checking':'not_available',...(resetDownstream?{directorAnswer:null,soleShareholderAnswer:null,ownershipDraft:null}:{}),updatedAt:now};
      }else if(step===2){
        const savedSetupAnswers=CompanyProfile.setupAnswersFor(profile);
        if(savedSetupAnswers.registrationAnswer!=='yes'||savedSetupAnswers.identityDetailsConfirmed!==true){
          return{status:'review_required',reviewReasons:['company_registration_required_before_bookkeeping'],data:{profile:clone(profile),draftState:clone(this.companyDraftState),officialFactsDeferred:true},nextRoute:'ltd.onboarding.registration-pending'};
        }
        const tradingAnswer=CompanyProfile.SETUP_ANSWER_VALUES.includes(values.tradingAnswer)?values.tradingAnswer:values.tradingStatus==='trading'?'yes':values.tradingStatus==='not_started'?'no':values.tradingStatus==='not_sure'?'not_sure':null;
        const tradingStatus=tradingAnswer==='yes'?'trading':tradingAnswer==='no'?'not_started':undefined,startDate=values.tradingStartDate;
        if(!CompanyProfile.SETUP_ANSWER_VALUES.includes(tradingAnswer))errors.push(fieldError('tradingStatus','answer_required'));
        if(!Domain.isoDate(profile.incorporationDate))errors.push(fieldError('incorporationDate','incorporation_date_required'));
        if(tradingAnswer==='not_sure'){
          if(errors.length)return{status:'field_error',fieldErrors:errors};
          next=clone(next);for(const field of ['tradingStatus','tradingStartDate','accountingReferenceDate','accountingPeriod','corporationTaxStatus','confirmedAt'])delete next[field];
          next=CompanyProfile.setSetupAnswers(next,{tradingAnswer:'not_sure'},{now,deviceId:this.deviceId});this.pendingPeriodPlan=null;
        }else{
        ...
        next=CompanyProfile.setSetupAnswers(next,{tradingAnswer},{now,deviceId:this.deviceId});
        }
      }else if(step===3){
        const founderName=cleanText(values.founderName,300),otherName=cleanText(values.otherShareholderName,300),founderShares=Number(values.founderShares),otherShares=values.otherShares==null?0:Number(values.otherShares),directorAnswer=values.directorAnswer,soleAnswer=CompanyProfile.SETUP_ANSWER_VALUES.includes(values.soleShareholderAnswer)?values.soleShareholderAnswer:CompanyProfile.SETUP_ANSWER_VALUES.includes(values.onlyShareholder)?values.onlyShareholder:(Number.isSafeInteger(otherShares)&&otherShares>0?'no':Number.isSafeInteger(founderShares)&&founderShares>0?'yes':null);
        if(!founderName)errors.push(fieldError('founderName','founder_name_required'));if(!CompanyProfile.SETUP_ANSWER_VALUES.includes(soleAnswer))errors.push(fieldError('onlyShareholder','answer_required'));if(soleAnswer!=='not_sure'&&(!Number.isSafeInteger(founderShares)||founderShares<=0))errors.push(fieldError('founderShares','founder_shares_invalid'));if(soleAnswer==='yes'&&(founderShares!==100||otherShares!==0))errors.push(fieldError('founderShares','share_ownership_total_required',{total:Number.isFinite(founderShares+otherShares)?founderShares+otherShares:0}));if(soleAnswer==='no'&&(!Number.isSafeInteger(otherShares)||otherShares<=0||founderShares+otherShares!==100))errors.push(fieldError('otherShares','share_ownership_total_required',{total:Number.isFinite(founderShares+otherShares)?founderShares+otherShares:0}));if(soleAnswer==='no'&&!otherName)errors.push(fieldError('otherShareholderName','other_shareholder_name_required'));if(!['yes','no','not_sure'].includes(directorAnswer))errors.push(fieldError('directorAnswer','director_answer_required'));
        if(errors.length)return{status:'field_error',fieldErrors:errors};
        next=CompanyProfile.setSetupAnswers(next,{directorAnswer,soleShareholderAnswer:soleAnswer},{now,deviceId:this.deviceId});
        if(soleAnswer==='not_sure'){
          next=clone(next);delete next.accountHolder;next.directors=[];next.shareholders=[];next.shareClasses=[];
          next=CompanyProfile.setSetupAnswers(next,{directorAnswer,soleShareholderAnswer:soleAnswer},{now,deviceId:this.deviceId});
        }else{
        ...
        this.companyDraftState={...this.companyDraftState,status:soleAnswer==='not_sure'?'ownership_review_required':directorAnswer==='yes'?'in_progress':'director_review_required',directorAnswer,soleShareholderAnswer:soleAnswer,ownershipDraft:clone(next.shareholders),updatedAt:now};
      }else if(step===4){
        ...
        const activityType=values.ordinaryServiceDigital===true?'service_digital':values.ordinaryServiceDigital===false?'not_service_digital':'not_sure';
        next=CompanyProfile.answer(next,'activity_profile',activityType,{now,deviceId:this.deviceId});
      ...
      const savedAnswers=CompanyProfile.setupAnswersFor(next);
      if(step===1&&savedAnswers.registrationAnswer!=='yes')return{status:'review_required',reviewReasons:[savedAnswers.registrationAnswer==='not_sure'?'company_registration_review_required':'company_registration_required_before_bookkeeping'],data:{profile:clone(next),draftState:clone(this.companyDraftState),officialFactsDeferred:true},nextRoute:'ltd.onboarding.registration-pending'};
      if(step===2&&savedAnswers.tradingAnswer==='not_sure')return{status:'review_required',reviewReasons:['trading_status_review_required'],data:{profile:clone(next),draftState:clone(this.companyDraftState),bookkeepingAllowed:false},nextRoute:'ltd.onboarding.trading-review'};
      if(step===3&&savedAnswers.soleShareholderAnswer==='not_sure')return{status:'review_required',reviewReasons:['sole_shareholder_review_required'],data:{profile:clone(next),draftState:clone(this.companyDraftState),bookkeepingAllowed:false},nextRoute:'ltd.onboarding.ownership-review'};
      if(step===3&&savedAnswers.registrationAnswer!=='yes')return{status:'review_required',reviewReasons:['company_registration_required_before_bookkeeping',...(this.companyDraftState.directorAnswer!=='yes'?['director_confirmation_required']:[])],data:{profile:clone(next),draftState:clone(this.companyDraftState),bookkeepingAllowed:false},nextRoute:'ltd.onboarding.registration-pending'};
  ...
  correctRecord(input={}){
    ...
    const source=prior.sourceTransaction||{};if(source.settlement||source.assetSnapshot||source.depreciationSnapshot)return{status:'field_error',fieldErrors:[fieldError('eventId','company_book_specific_correction_required')],data:{requiredCallbacks:['onCorrectSalesInvoice','onVoidSalesInvoice','onCorrectSupplierBill','onVoidSupplierBill','onCorrectCompanyAsset','onVoidCompanyAsset','onReverseCompanyBookPayment','onReverseCompanyDepreciation']}};
    ...
    const profile=this.activeProfile(),replacement=input.replacement||{},type=source.companyTransactionType,date=replacement.date||source.date,amountMinor=Number.isSafeInteger(replacement.amountMinor)?replacement.amountMinor:source.amountMinor,description=cleanText(replacement.description||source.purpose,300);
  ...
  createSalesInvoice(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};
    try{const invoice={...clone(input.invoice||{}),id:input.invoice&&input.invoice.id||this.newId('sales-invoice')},result=CompanyBooks.createSalesInvoice({profile,currentEvents:this.eventsFor(profile),currentInvoices:this.recordsFor('salesInvoices',profile),invoice,confirmations:clone(input.taxFacts||{}),now:this.now(),deviceId:this.deviceId});this.saveBookResult('salesInvoices',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.sales-invoices'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('invoice',stableReason(error,'sales_invoice_facts_incomplete'))]};}
  }
  recordSalesInvoicePayment(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile(),invoice=this.recordsFor('salesInvoices',profile).find(record=>record.id===input.invoiceId);if(!invoice)return{status:'field_error',fieldErrors:[fieldError('invoiceId','sales_invoice_not_found')]};this.ensurePaymentAccounts(profile);
    try{const payment={...clone(input.payment||{}),id:input.payment&&input.payment.id||this.newId('sales-invoice-payment'),companyPaymentAccountId:this.account(profile,'entity').id},result=CompanyBooks.paySalesInvoice({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),invoice,payment,now:this.now(),deviceId:this.deviceId});this.saveBookResult('salesInvoices',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.sales-invoices'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('payment',stableReason(error,'sales_invoice_payment_facts_incomplete'))]};}
  }
  correctSalesInvoice(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),invoice=this.recordsFor('salesInvoices',profile).find(record=>record.id===input.invoiceId);if(!invoice)return{status:'field_error',fieldErrors:[fieldError('invoiceId','sales_invoice_not_found')]};try{const result=CompanyBooks.correctSalesInvoice({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),invoice,replacement:clone(input.replacement||input.correction||input.correctedFacts||{}),confirmations:clone(input.taxFacts||{}),reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),correctionId:input.correctionId||this.newId('sales-invoice-correction'),reversalEventId:input.reversalEventId||this.newId('sales-invoice-reversal'),now:this.now(),deviceId:this.deviceId});if(result.record)this.saveBookResult('salesInvoices',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.sales-invoices'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('invoice',stableReason(error,'sales_invoice_correction_failed'))]};}}
  voidSalesInvoice(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),record=this.recordsFor('salesInvoices',profile).find(item=>item.id===input.invoiceId);if(!record)return{status:'field_error',fieldErrors:[fieldError('invoiceId','sales_invoice_not_found')]};try{const result=CompanyBooks.voidSettlementRecord({profile,currentEvents:this.eventsFor(profile),record,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),reversalEventId:input.reversalEventId||this.newId('sales-invoice-void'),now:this.now(),deviceId:this.deviceId});this.saveBookResult('salesInvoices',result);return{status:'ok',data:clone(result),nextRoute:'ltd.money.sales-invoices'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('invoice',stableReason(error,'sales_invoice_void_failed'))]};}}
  createSupplierBill(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};
    try{const bill={...clone(input.bill||{}),id:input.bill&&input.bill.id||this.newId('supplier-bill')},result=CompanyBooks.createSupplierBill({profile,currentEvents:this.eventsFor(profile),currentBills:this.recordsFor('supplierBills',profile),bill,confirmations:clone(input.taxFacts||{}),now:this.now(),deviceId:this.deviceId});this.saveBookResult('supplierBills',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.supplier-bills'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('bill',stableReason(error,'supplier_bill_facts_incomplete'))]};}
  }
  recordSupplierBillPayment(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile(),bill=this.recordsFor('supplierBills',profile).find(record=>record.id===input.billId);if(!bill)return{status:'field_error',fieldErrors:[fieldError('billId','supplier_bill_not_found')]};this.ensurePaymentAccounts(profile);
    try{const payment={...clone(input.payment||{}),id:input.payment&&input.payment.id||this.newId('supplier-bill-payment'),companyPaymentAccountId:this.account(profile,'entity').id},result=CompanyBooks.paySupplierBill({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),bill,payment,now:this.now(),deviceId:this.deviceId});this.saveBookResult('supplierBills',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.supplier-bills'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('payment',stableReason(error,'supplier_bill_payment_facts_incomplete'))]};}
  }
  correctSupplierBill(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),bill=this.recordsFor('supplierBills',profile).find(record=>record.id===input.billId);if(!bill)return{status:'field_error',fieldErrors:[fieldError('billId','supplier_bill_not_found')]};try{const result=CompanyBooks.correctSupplierBill({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),bill,replacement:clone(input.replacement||input.correction||input.correctedFacts||{}),confirmations:clone(input.taxFacts||{}),reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),correctionId:input.correctionId||this.newId('supplier-bill-correction'),reversalEventId:input.reversalEventId||this.newId('supplier-bill-reversal'),now:this.now(),deviceId:this.deviceId});if(result.record)this.saveBookResult('supplierBills',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.supplier-bills'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('bill',stableReason(error,'supplier_bill_correction_failed'))]};}}
  voidSupplierBill(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),record=this.recordsFor('supplierBills',profile).find(item=>item.id===input.billId);if(!record)return{status:'field_error',fieldErrors:[fieldError('billId','supplier_bill_not_found')]};try{const result=CompanyBooks.voidSettlementRecord({profile,currentEvents:this.eventsFor(profile),record,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),reversalEventId:input.reversalEventId||this.newId('supplier-bill-void'),now:this.now(),deviceId:this.deviceId});this.saveBookResult('supplierBills',result);return{status:'ok',data:clone(result),nextRoute:'ltd.money.supplier-bills'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('bill',stableReason(error,'supplier_bill_void_failed'))]};}}
  registerCompanyAsset(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};this.ensurePaymentAccounts(profile);
    try{const asset={...clone(input.asset||{}),id:input.asset&&input.asset.id||this.newId('fixed-asset'),companyPaymentAccountId:this.account(profile,'entity').id},result=CompanyBooks.registerFixedAsset({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),currentAssets:this.recordsFor('fixedAssets',profile),asset,now:this.now(),deviceId:this.deviceId});this.saveBookResult('fixedAssets',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('asset',stableReason(error,'fixed_asset_facts_incomplete'))]};}
  }
  recordCompanyAssetPayment(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile(),asset=this.recordsFor('fixedAssets',profile).find(record=>record.id===input.assetId);if(!asset)return{status:'field_error',fieldErrors:[fieldError('assetId','fixed_asset_not_found')]};this.ensurePaymentAccounts(profile);
    try{const payment={...clone(input.payment||{}),id:input.payment&&input.payment.id||this.newId('fixed-asset-payment'),companyPaymentAccountId:this.account(profile,'entity').id},result=CompanyBooks.payFixedAsset({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),asset,payment,now:this.now(),deviceId:this.deviceId});this.saveBookResult('fixedAssets',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('payment',stableReason(error,'fixed_asset_payment_facts_incomplete'))]};}
  }
  recordCompanyDepreciation(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile(),asset=this.recordsFor('fixedAssets',profile).find(record=>record.id===input.assetId);if(!asset)return{status:'field_error',fieldErrors:[fieldError('assetId','fixed_asset_not_found')]};
    try{const result=CompanyBooks.recordDepreciation({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),asset,periodStartDate:input.periodStartDate||profile.accountingPeriod.startDate,periodEndDate:input.periodEndDate||profile.accountingPeriod.endDate,asOfDate:this.currentDate(),now:this.now(),deviceId:this.deviceId});this.saveBookResult('fixedAssets',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('depreciation',stableReason(error,'depreciation_facts_incomplete'))]};}
  }
  correctCompanyAsset(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),asset=this.recordsFor('fixedAssets',profile).find(record=>record.id===input.assetId);if(!asset)return{status:'field_error',fieldErrors:[fieldError('assetId','fixed_asset_not_found')]};this.ensurePaymentAccounts(profile);try{const result=CompanyBooks.correctFixedAsset({profile,currentEvents:this.eventsFor(profile),paymentAccounts:this.list('paymentAccounts'),companyPaymentAccountId:this.account(profile,'entity').id,asset,replacement:clone(input.replacement||input.correction||input.correctedFacts||{}),reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),correctionId:input.correctionId||this.newId('fixed-asset-correction'),reversalEventId:input.reversalEventId||this.newId('fixed-asset-reversal'),now:this.now(),deviceId:this.deviceId});if(result.record)this.saveBookResult('fixedAssets',result);return{status:['posted','existing'].includes(result.status)?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('asset',stableReason(error,'fixed_asset_correction_failed'))]};}}
  voidCompanyAsset(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),asset=this.recordsFor('fixedAssets',profile).find(record=>record.id===input.assetId);if(!asset)return{status:'field_error',fieldErrors:[fieldError('assetId','fixed_asset_not_found')]};try{const result=CompanyBooks.voidFixedAsset({profile,currentEvents:this.eventsFor(profile),asset,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),reversalEventId:input.reversalEventId||this.newId('fixed-asset-void'),now:this.now(),deviceId:this.deviceId});this.saveBookResult('fixedAssets',result);return{status:'ok',data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('asset',stableReason(error,'fixed_asset_void_failed'))]};}}
  reverseCompanyBookPayment(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),mapping={sales_invoice:'salesInvoices',supplier_bill:'supplierBills',fixed_asset:'fixedAssets'},collection=mapping[input.recordType],record=collection&&this.recordsFor(collection,profile).find(item=>item.id===input.recordId);if(!record)return{status:'field_error',fieldErrors:[fieldError('recordId','company_book_record_not_found')]};try{const result=CompanyBooks.reverseBookPayment({profile,currentEvents:this.eventsFor(profile),record,paymentId:input.paymentId,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),reversalEventId:input.reversalEventId||this.newId('company-payment-reversal'),now:this.now(),deviceId:this.deviceId});this.saveBookResult(collection,result);return{status:'ok',data:clone(result),nextRoute:input.recordType==='fixed_asset'?'ltd.records.company-assets':'ltd.workspace.money'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('paymentId',stableReason(error,'company_payment_reversal_failed'))]};}}
  reverseCompanyDepreciation(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),asset=this.recordsFor('fixedAssets',profile).find(record=>record.id===input.assetId);if(!asset)return{status:'field_error',fieldErrors:[fieldError('assetId','fixed_asset_not_found')]};try{const result=CompanyBooks.reverseDepreciation({profile,currentEvents:this.eventsFor(profile),asset,depreciationId:input.depreciationId,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),reversalEventId:input.reversalEventId||this.newId('depreciation-reversal'),now:this.now(),deviceId:this.deviceId});this.saveBookResult('fixedAssets',result);return{status:'ok',data:clone(result),nextRoute:'ltd.records.company-assets'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('depreciationId',stableReason(error,'depreciation_reversal_failed'))]};}}
  matchBankStatement(input={}){
    this.requireAccess('create_event');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};this.ensurePaymentAccounts(profile);
    try{const statement={...clone(input.statement||{}),id:input.statement&&input.statement.id||this.newId('bank-reconciliation'),accountId:this.account(profile,'entity').id},result=CompanyBooks.reconcileBankStatement({profile,currentEvents:this.eventsFor(profile),currentReconciliations:this.recordsFor('bankReconciliations',profile),statement,asOfDate:this.currentDate(),now:this.now(),deviceId:this.deviceId});this.atomic(()=>this.upsert('bankReconciliations',result.record));return{status:result.status==='reconciled'?'ok':'review_required',reviewReasons:result.reasonCodes||[],data:clone(result),nextRoute:'ltd.money.bank-matching'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('statement',stableReason(error,'bank_statement_facts_incomplete'))]};}
  }
  deleteBankReconciliation(input={}){this.requireAccess('correct_event');const profile=this.activeProfile(),reconciliation=this.recordsFor('bankReconciliations',profile).find(record=>record.id===input.reconciliationId);if(!reconciliation)return{status:'field_error',fieldErrors:[fieldError('reconciliationId','bank_reconciliation_not_found')]};try{const result=CompanyBooks.voidBankReconciliation({profile,reconciliation,reasonCode:input.reasonCode||input.reason,evidenceRefs:clone(input.evidenceRefs||[]),now:this.now(),deviceId:this.deviceId});this.atomic(()=>this.upsert('bankReconciliations',result.record));return{status:'ok',data:clone(result),nextRoute:'ltd.money.bank-matching'};}catch(error){return{status:'field_error',fieldErrors:[fieldError('reconciliationId',stableReason(error,'bank_reconciliation_delete_failed'))]};}}
  prepareCompanyYear(input={}){this.requireAccess('generate_working_pack');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};try{const pack=CompanyBooks.buildSelfFilingPack(this.statutoryInput(profile,input.filingFacts||{}));return{status:pack.status==='ready_for_director_check'?'ok':'review_required',reviewReasons:pack.reasonCodes,data:{figures:pack.accounts,statutoryChecklist:pack.statutoryChecklist,readiness:pack.readiness},nextRoute:'ltd.tax.company-year'};}catch(error){return{status:'review_required',reviewReasons:[stableReason(error,'company_year_figures_need_checking')],data:null,nextRoute:'ltd.tax.company-year'};}}
  selfFilingPack(input={}){this.requireAccess('generate_working_pack');const profile=this.activeProfile();if(!profile)return{status:'field_error',fieldErrors:[fieldError('company','company_not_found')]};try{const pack=CompanyBooks.buildSelfFilingPack(this.statutoryInput(profile,input.filingFacts||{}));return{status:pack.status==='ready_for_director_check'?'ok':'review_required',reviewReasons:pack.reasonCodes,data:{fileName:`taxmate-${String(profile.legalName||'company').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-self-filing-pack.json`,mimeType:'application/json',payload:pack},nextRoute:'ltd.tax.self-filing-pack'};}catch(error){return{status:'review_required',reviewReasons:[stableReason(error,'self_filing_pack_needs_checking')],data:null,nextRoute:'ltd.tax.self-filing-pack'};}}
