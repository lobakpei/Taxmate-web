...
A Founder promotion redeemed directly in TaxMate grants a server-controlled fixed-period or permanent Plus or Pro entitlement. It requires no card, creates no Stripe subscription and never starts an automatic charge when it expires. A fixed campaign always ends at its stated campaign expiry even if redeemed later. After expiry or a specific revocation, TaxMate recalculates all remaining paid and promotional entitlements and applies the highest active tier; otherwise access becomes Free. Disabling a code stops future redemptions but does not cancel grants already redeemed. Promotions may have eligibility, start-date, redemption-count and expiry conditions disclosed with the offer or at redemption. Account and bookkeeping records remain preserved when access changes.</p>
