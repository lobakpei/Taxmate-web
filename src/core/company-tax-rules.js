(function(root,factory){
  ...
  const VERIFIED_AT='2026-08-22',RULESET_VERSION='uk-company-tax.2026-08-22.2';
  ...
  const rate=Object.freeze({smallRate:Object.freeze({numerator:19,denominator:100}),mainRate:Object.freeze({numerator:25,denominator:100}),lowerLimitMinor:5000000,upperLimitMinor:25000000,marginalFraction:Object.freeze({numerator:3,denominator:200}),financialYearDays:365});
  const RULESET=Object.freeze({
    schemaVersion:1,rulesetVersion:RULESET_VERSION,jurisdiction:'UK',verifiedAt:VERIFIED_AT,effectiveFrom:'2025-04-01',effectiveTo:'2027-03-31',reviewBy:'2027-03-31',
    officialSources:Object.freeze(Object.values(SOURCES)),supportedFinancialYears:Object.freeze([
      Object.freeze({id:'FY2025',startDate:'2025-04-01',endDate:'2026-03-31',...rate}),
      Object.freeze({id:'FY2026',startDate:'2026-04-01',endDate:'2027-03-31',...rate})
    ]),
    supportedPredicates:Object.freeze({ukResident:true,ringFenceProfits:false,closeInvestmentHoldingCompany:false,associatedCompanies:0,qualifyingDistributionsMinor:0,currency:'GBP',lossesArisingOnOrAfter:'2017-04-01',maximumSimpleLossProfitMinor:500000000}),
  ...
  function validateRuleset(value){
    ...
    if(value&&(value.verifiedAt!==VERIFIED_AT||value.effectiveFrom!=='2025-04-01'||value.effectiveTo!=='2027-03-31'||value.reviewBy!=='2027-03-31'))errors.push('ruleset dates changed');
    ...
    if(!Array.isArray(value&&value.supportedFinancialYears)||value.supportedFinancialYears.length!==2)errors.push('financial years are required');else for(const [index,year] of value.supportedFinancialYears.entries()){try{const expected=[{id:'FY2025',startDate:'2025-04-01',endDate:'2026-03-31'},{id:'FY2026',startDate:'2026-04-01',endDate:'2027-03-31'}][index];if(!keys(year,['id','startDate','endDate','smallRate','mainRate','lowerLimitMinor','upperLimitMinor','marginalFraction','financialYearDays'])||year.id!==expected.id||year.startDate!==expected.startDate||year.endDate!==expected.endDate||year.lowerLimitMinor!==5000000||year.upperLimitMinor!==25000000||year.financialYearDays!==365)throw new Error('invalid');ratio(year.smallRate,'small rate');ratio(year.mainRate,'main rate');ratio(year.marginalFraction,'marginal fraction');if(year.smallRate.numerator!==19||year.smallRate.denominator!==100||year.mainRate.numerator!==25||year.mainRate.denominator!==100||year.marginalFraction.numerator!==3||year.marginalFraction.denominator!==200)throw new Error('changed');}catch(_){errors.push('financial year is invalid');}}
    const predicates=value&&value.supportedPredicates;if(!plain(predicates)||predicates.ukResident!==true||predicates.ringFenceProfits!==false||predicates.closeInvestmentHoldingCompany!==false||predicates.associatedCompanies!==0||predicates.qualifyingDistributionsMinor!==0||predicates.currency!=='GBP'||predicates.lossesArisingOnOrAfter!=='2017-04-01'||predicates.maximumSimpleLossProfitMinor!==500000000)errors.push('supported predicates changed');
  ...
  function calculateEstimate(taxableProfitMinor,augmentedProfitMinor,periodDays,year){
    ...
    const n=BigInt(taxableProfitMinor),a=BigInt(augmentedProfitMinor),days=BigInt(periodDays),yearDays=BigInt(year.financialYearDays),lower=BigInt(year.lowerLimitMinor),upper=BigInt(year.upperLimitMinor),belowOrAtLower=a*yearDays<=lower*days,atOrAboveUpper=a*yearDays>=upper*days;
    const lowerThresholdMinor=roundRationalMinor(lower*days,yearDays),upperThresholdMinor=roundRationalMinor(upper*days,yearDays);let band,taxMinor,marginalReliefMinor=0,mainRateTaxMinor=roundRationalMinor(n*BigInt(year.mainRate.numerator),BigInt(year.mainRate.denominator));
    if(taxableProfitMinor===0){band='zero';taxMinor=0;}else if(belowOrAtLower){band='small_profits';taxMinor=roundRationalMinor(n*BigInt(year.smallRate.numerator),BigInt(year.smallRate.denominator));}else if(atOrAboveUpper){band='main_rate';taxMinor=mainRateTaxMinor;}else{
      band='marginal_relief';const gapNumerator=upper*days-a*yearDays,reliefNumerator=BigInt(year.marginalFraction.numerator)*gapNumerator*n,reliefDenominator=BigInt(year.marginalFraction.denominator)*yearDays*a;marginalReliefMinor=roundRationalMinor(reliefNumerator,reliefDenominator);taxMinor=mainRateTaxMinor-marginalReliefMinor;
    }
    return{band,taxableProfitMinor,augmentedProfitMinor,lowerThresholdMinor,upperThresholdMinor,mainRateTaxMinor,marginalReliefMinor,corporationTaxEstimateMinor:taxMinor,formulaVersion:RULESET_VERSION,roundingPolicy:RULESET.estimateRoundingPolicy};
  }
