...
const required=['STRIPE_SECRET_KEY','STRIPE_WEBHOOK_SECRET','TAXMATE_STRIPE_ACCOUNT_ID','STRIPE_PLUS_MONTHLY_PRICE_ID','STRIPE_PLUS_ANNUAL_PRICE_ID','STRIPE_PRO_MONTHLY_PRICE_ID','STRIPE_PRO_ANNUAL_PRICE_ID'];
...
const prices=priceNames.map(name=>process.env[name]);if(prices.some(id=>!/^price_[A-Za-z0-9]+$/.test(id))||new Set(prices).size!==prices.length){console.error('Invalid TaxMate Stripe price identities');process.exit(2);}
...
let result;
try{
  fs.writeFileSync(generatedEnv,`STRIPE_PLUS_MONTHLY_PRICE_ID=${env.STRIPE_PLUS_MONTHLY_PRICE_ID}\nSTRIPE_PLUS_ANNUAL_PRICE_ID=${env.STRIPE_PLUS_ANNUAL_PRICE_ID}\nSTRIPE_PRO_MONTHLY_PRICE_ID=${env.STRIPE_PRO_MONTHLY_PRICE_ID}\nSTRIPE_PRO_ANNUAL_PRICE_ID=${env.STRIPE_PRO_ANNUAL_PRICE_ID}\nSTRIPE_PLUS_LEGACY_PRICE_IDS=${env.STRIPE_PLUS_LEGACY_PRICE_IDS||''}\nSTRIPE_PRO_LEGACY_PRICE_IDS=${env.STRIPE_PRO_LEGACY_PRICE_IDS||''}\nPUBLIC_APP_URL=${env.PUBLIC_APP_URL}\n`,'utf8');
