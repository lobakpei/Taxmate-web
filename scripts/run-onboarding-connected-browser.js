...
const connectedHomeOnly=process.argv.includes('--connected-home-only')||process.env.TAXMATE_CONNECTED_HOME_ONLY==='1';
const firstSyncOnly=process.argv.includes('--first-sync-only')||process.env.TAXMATE_FIRST_SYNC_ONLY==='1';
const authOnceOnly=process.argv.includes('--auth-once-only')||process.env.TAXMATE_AUTH_ONCE_ONLY==='1';
const visualRepaintOnly=process.argv.includes('--visual-repaint-only')||process.env.TAXMATE_VISUAL_REPAINT_ONLY==='1';
const runtime=localToolEnvironment(root);
const env={...runtime.env,NODE_PATH:localNodePath(root),FUNCTIONS_DISCOVERY_TIMEOUT:'60000',COMPANIES_HOUSE_API_KEY:'emulator-placeholder',STRIPE_SECRET_KEY:'emulator-placeholder',STRIPE_WEBHOOK_SECRET:'emulator-placeholder',STRIPE_PLUS_MONTHLY_PRICE_ID:'price_plus_monthly_emulator',STRIPE_PLUS_ANNUAL_PRICE_ID:'price_plus_annual_emulator',STRIPE_PRO_MONTHLY_PRICE_ID:'price_pro_monthly_emulator',STRIPE_PRO_ANNUAL_PRICE_ID:'price_pro_annual_emulator',STRIPE_PLUS_LEGACY_PRICE_IDS:'',STRIPE_PRO_LEGACY_PRICE_IDS:'',PUBLIC_APP_URL:'http://127.0.0.1:4180',TAXMATE_ONBOARDING_EVIDENCE:evidence,TAXMATE_ONBOARDING_REVIEW:reviewMode?'1':'0',TAXMATE_CONNECTED_HOME_ONLY:connectedHomeOnly?'1':'0',TAXMATE_FIRST_SYNC_ONLY:firstSyncOnly?'1':'0',TAXMATE_AUTH_ONCE_ONLY:authOnceOnly?'1':'0',TAXMATE_VISUAL_REPAINT_ONLY:visualRepaintOnly?'1':'0'};
// Every real cloud mutation now exercises server receipt admission, including
// first-sync, auth-once and Assistant subsets. Never substitute a fake permit.
const emulatorSet='auth,firestore,storage,functions';
const isolated=prepare(root),command=`"${firebase}" emulators:exec${isolated.arg} --project demo-taxmate --only ${emulatorSet} "node tests/browser/onboarding-connected.e2e.js"`;
