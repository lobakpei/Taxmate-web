...
function build(output){
  ...
  const filename='taxmate-test-terms-20260914.html',manifest={status:'LOCAL_ONLY_NOT_PUBLISHED',contractVersion:contract.version,contractSha256:crypto.createHash('sha256').update(contract.termsHtml).digest('hex'),htmlSha256:crypto.createHash('sha256').update(html).digest('hex'),filename,supplierEstablishmentAddressVerified:false,liveSalesCleared:false,stripeSettingsChanged:false,proposedProjectId:'taxmate-staging',hostingTargetCurrentlyVerified:false,proposedChannel:'billing-terms-20260914',proposedExpiryDays:7,publicUrl:null};
