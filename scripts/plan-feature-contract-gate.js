...
const expected={mileageCompare:'plus',multiBiz:'plus',receiptPhoto:'plus',pdfReport:'plus',partnerSync:'pro',sa104:'pro',receiptPack:'plus',mtdReady:'pro',ltd:'pro'};
...
assert.match(app,/const plus = \['feat\.multiBiz','feat\.receiptPhoto','feat\.receiptPack','feat\.mileageCompare','feat\.pdfReport'\]/);
assert.match(app,/const pro  = \['feat\.partnerSync','feat\.sa104','feat\.mtdReady','feat\.ltd'\]/);
...
for(const phrase of ['multiple businesses','receipt photos','mileage comparison','PDF tax report','Partner Sync','SA104 partnership working paper','Receipt Pack PDF','MTD quarterly record preparation'])assert.match(help,new RegExp(phrase,'i'));
...
assert.match(help,/standard price of £11\.99 struck through beside the £9\.99 launch price/i);
assert.match(contract,/standard £11\.99 price struck through beside the £9\.99 launch price/i);
assert.match(help,/MTD quarterly updates[\s\S]*HMRC-compatible submission route/i);
...
assert.match(firestore,/allow create, update: if member\(partnershipId\) && memberRecordWritable\(partnershipId\) && safeSharedReceipt\(\) && receiptAdmitted\([^\n]+ && pro\(request\.auth\.uid\)/);
assert.match(storage,/allow create:[^\n]*receiptAccess\(uid\)/);
