...
const expected={mileageCompare:'plus',multiBiz:'plus',receiptPhoto:'plus',pdfReport:'plus',partnerSync:'pro',sa104:'pro',receiptPack:'pro',mtdReady:'pro'};
...
assert.match(app,/const pro  = \['feat\.partnerSync','feat\.sa104','feat\.receiptPack','feat\.mtdReady'\]/);
...
for(const price of ['£3.99','£29.99','£7.99','£59.99']){assert.match(app,new RegExp(price.replace('.','\\.')));assert.match(help,new RegExp(price.replace('.','\\.')));assert.match(contract,new RegExp(price.replace('.','\\.')));}
...
console.log('PLAN_FEATURE_CONTRACT_GATE PASS features=20 plan_tiers=3 receipt_locales=6 server_enforced=2');
