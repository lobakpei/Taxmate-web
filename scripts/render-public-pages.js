...
for(const kind of ['help','privacy','delete-account','terms']){const file=path.join(root,kind+'.html'),expected=Content.publicPage(kind),actual=fs.existsSync(file)?fs.readFileSync(file,'utf8'):'';if(actual===expected)continue;if(write)fs.writeFileSync(file,expected);else{console.error(`${kind}.html is not generated from src/core/product-content.js`);mismatch=true;}}
