...
async function rasterOpticalIcon(page,svg,size,output,maskable=false){
  ...
  const targetWidth=size<=16?0.76:size<=32?0.79:0.80,targetCenterX=0.535,targetCenterY=0.505,scale=maskable?Math.min(0.63/baseWidth,0.63/baseHeight):targetWidth/baseWidth,left=((maskable?0.5:targetCenterX)-sourceCenterX*scale)*size,top=((maskable?0.5:targetCenterY)-sourceCenterY*scale)*size;
...
async function scanRaster(page,file,background){
  return page.evaluate(async input=>{const image=await new Promise((resolve,reject)=>{const value=new Image();value.onload=()=>resolve(value);value.onerror=reject;value.src=input.src;}),canvas=document.createElement('canvas');canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;const context=canvas.getContext('2d',{willReadFrequently:true});context.drawImage(image,0,0);const data=context.getImageData(0,0,canvas.width,canvas.height).data,bg=input.background,bounds={left:canvas.width,top:canvas.height,right:-1,bottom:-1};let transparentPixels=0,opaquePixels=0,foregroundPixels=0,blackPixels=0;for(let y=0;y<canvas.height;y++)for(let x=0;x<canvas.width;x++){const index=(y*canvas.width+x)*4,alpha=data[index+3];if(alpha===0)transparentPixels++;if(alpha===255)opaquePixels++;if(alpha>180&&data[index]<48&&data[index+1]<48&&data[index+2]<48)blackPixels++;const delta=Math.max(Math.abs(data[index]-bg[0]),Math.abs(data[index+1]-bg[1]),Math.abs(data[index+2]-bg[2]));if(alpha>0&&delta>12){foregroundPixels++;bounds.left=Math.min(bounds.left,x);bounds.top=Math.min(bounds.top,y);bounds.right=Math.max(bounds.right,x);bounds.bottom=Math.max(bounds.bottom,y);}}return{width:canvas.width,height:canvas.height,transparentPixels,opaquePixels,foregroundPixels,blackPixels,foregroundBounds:bounds,foregroundBoundsRatio:{left:bounds.left/canvas.width,top:bounds.top/canvas.height,right:(bounds.right+1)/canvas.width,bottom:(bounds.bottom+1)/canvas.height}};},{src:'data:image/png;base64,'+read(file).toString('base64'),background});
}
function ico(images){
...
async function main(){
  ...
  try{
    ...
    for(const [name,value] of Object.entries(rasterMeasurements)){const width=value.foregroundBoundsRatio.right-value.foregroundBoundsRatio.left,minBlack=value.width<=16?1:2;if(width<0.75||width>0.83||value.foregroundBounds.left<=0||value.foregroundBounds.top<=0||value.foregroundBounds.right>=value.width-1||value.foregroundBounds.bottom>=value.height-1||value.blackPixels<minBlack)throw new Error(`Ordinary icon optical/face validation failed for ${name}: ${JSON.stringify(value)}`);}
