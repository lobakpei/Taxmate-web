...
function transparentIcon(svg){
  const icon=svgParts(svg),foreground=icon.paths.filter(source=>pathAttribute(source,'fill')!=='#fdfdfd');
  if(icon.paths.length!==3||foreground.length!==2)throw new Error('Light App Icon layers drifted');
  return `${icon.open}\n${foreground.join('\n')}\n</svg>\n`;
}
...
async function rasterOpticalIcon(page,svg,size,output,maskable=false){
  ...
  const small=size<=48,targetWidth=small?0.82:0.68,targetCenterX=0.50,targetCenterY=0.50,scale=maskable?Math.min(0.55/baseWidth,0.55/baseHeight):targetWidth/baseWidth,left=(targetCenterX-sourceCenterX*scale)*size,top=(targetCenterY-sourceCenterY*scale)*size,background=small?'transparent':'#0f1620';
  await page.setViewportSize({width:size,height:size});
  await page.setContent(`<style>html,body{margin:0;width:${size}px;height:${size}px;background:${background};overflow:hidden}img{position:absolute;display:block;width:${size*scale}px;height:${size*scale}px;left:${left}px;top:${top}px;max-width:none}</style><img id="asset" alt="" src="${dataUrl(svg)}">`);
  await page.screenshot({path:output,omitBackground:small,animations:'disabled',clip:{x:0,y:0,width:size,height:size}});
...
async function scanRaster(page,file,background){
  return page.evaluate(async input=>{const image=await new Promise((resolve,reject)=>{const value=new Image();value.onload=()=>resolve(value);value.onerror=reject;value.src=input.src;}),canvas=document.createElement('canvas');canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;const context=canvas.getContext('2d',{willReadFrequently:true});context.drawImage(image,0,0);const data=context.getImageData(0,0,canvas.width,canvas.height).data,bg=input.background,bounds={left:canvas.width,top:canvas.height,right:-1,bottom:-1};let transparentPixels=0,opaquePixels=0,foregroundPixels=0,blackPixels=0;for(let y=0;y<canvas.height;y++)for(let x=0;x<canvas.width;x++){const index=(y*canvas.width+x)*4,alpha=data[index+3];if(alpha===0)transparentPixels++;if(alpha===255)opaquePixels++;if(alpha>180&&data[index]<96&&data[index+1]<96&&data[index+2]<96)blackPixels++;const delta=Math.max(Math.abs(data[index]-bg[0]),Math.abs(data[index+1]-bg[1]),Math.abs(data[index+2]-bg[2]));if(alpha>180&&delta>12){foregroundPixels++;bounds.left=Math.min(bounds.left,x);bounds.top=Math.min(bounds.top,y);bounds.right=Math.max(bounds.right,x);bounds.bottom=Math.max(bounds.bottom,y);}}return{width:canvas.width,height:canvas.height,transparentPixels,opaquePixels,foregroundPixels,blackPixels,foregroundBounds:bounds,foregroundBoundsRatio:{left:bounds.left/canvas.width,top:bounds.top/canvas.height,right:(bounds.right+1)/canvas.width,bottom:(bounds.bottom+1)/canvas.height}};},{src:'data:image/png;base64,'+read(file).toString('base64'),background});
}
function ico(images){
...
async function main(){
  ...
  try{
    ...
    const rasterSource=transparentIcon(source.light_icon),iconOutputs=[[16,'favicon-16x16.png'],[32,'favicon-32x32.png'],[48,'favicon-48x48.png'],[180,'apple-touch-icon.png'],[192,'icon-192.png'],[512,'icon-512.png']],pngs={};
    for(const [size,name] of iconOutputs)pngs[name]=await rasterOpticalIcon(page,rasterSource,size,path.join(root,name),false);
    pngs['icon-512-maskable.png']=await rasterOpticalIcon(page,rasterSource,512,path.join(root,'icon-512-maskable.png'),true);
    write(path.join(root,'favicon.ico'),ico([16,32,48].map(size=>({size,png:pngs[`favicon-${size}x${size}.png`]}))));
    const rasterMeasurements={};for(const [,name] of iconOutputs)rasterMeasurements[name]=await scanRaster(page,path.join(root,name),name.startsWith('favicon-')?[0,0,0]:[15,22,32]);
    for(const [name,value] of Object.entries(rasterMeasurements)){const width=value.foregroundBoundsRatio.right-value.foregroundBoundsRatio.left,small=value.width<=48,minWidth=small?0.74:0.63,maxWidth=small?0.90:0.72,minBlack=value.width<=16?2:3,edgeInvalid=value.foregroundBounds.left<=0||value.foregroundBounds.top<=0||value.foregroundBounds.right>=value.width-1||value.foregroundBounds.bottom>=value.height-1;if(width<minWidth||width>maxWidth||edgeInvalid||(small&&value.blackPixels<minBlack))throw new Error(`Ordinary icon optical/face validation failed for ${name}: ${JSON.stringify(value)}`);}
    const maskable=await scanRaster(page,path.join(root,'icon-512-maskable.png'),[15,22,32]);
