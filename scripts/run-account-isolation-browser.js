...
const isolated=prepare(root),command=`"${firebase}" emulators:exec${isolated.arg} --project demo-taxmate --only auth,firestore,storage,functions "node tests/browser/account-isolation.e2e.js"`;
const child=spawnSync(command,{cwd:root,env,stdio:'inherit',shell:true});if(!hadEnv&&fs.existsSync(generatedEnv))fs.unlinkSync(generatedEnv);isolated.cleanup();process.exit(child.status==null?1:child.status);
