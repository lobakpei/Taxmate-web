...
const {VERSIONS}=require(path.join(root,'src','core','versions.js'));
...
const builtVersions=require(path.join(hostingRoot,'src','core','versions.js')).VERSIONS;
assert.deepEqual(builtVersions,VERSIONS,'production build must preserve the canonical version contract');
assert.equal(VERSIONS.APP_VERSION,JSON.parse(read('package.json')).version,'app and package versions must agree');
assert.match(VERSIONS.BUILD_ID,/^[A-Za-z0-9][A-Za-z0-9._-]+$/,'build identity must be explicit');
assert.match(VERSIONS.PWA_CACHE_VERSION,/^taxmate-v2-[A-Za-z0-9._-]+$/,'PWA cache identity must be explicit');
const escapedCache=VERSIONS.PWA_CACHE_VERSION.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
assert.match(fs.readFileSync(path.join(hostingRoot,'sw.js'),'utf8'),new RegExp(`const CACHE = ['"]${escapedCache}['"]`),'service worker cache must match the canonical version contract');
