...
function createService({db,client,targetPrice,customerFor,appUrl,moneyOperationsEnabled=false,consumerDisclosuresReady=false,now=Date.now}){
  ...
  async function checkout(user,data){
    ...
    let session;
    try{session=await client.checkout.sessions.create({mode:'subscription',adaptive_pricing:{enabled:false},customer,line_items:[{price:offer.priceId,quantity:1}],allow_promotion_codes:true,automatic_tax:{enabled:false},billing_address_collection:'required',consent_collection:{terms_of_service:'required'},success_url:appUrl+'?billing=success',cancel_url:appUrl+'?billing=cancelled',metadata:{firebaseUid:user.uid,taxmateOffer:offer.id,termsVersion:offer.termsVersion},subscription_data:{metadata:{firebaseUid:user.uid,tier:offer.tier,billingCadence:offer.cadence,taxmateOffer:offer.id}}},{idempotencyKey:operation.key});}
