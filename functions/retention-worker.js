...
async function acquire(db,uid,now,leaseMs){
 const ref=db.doc(`users/${uid}/retention/current`),entRef=db.doc(`users/${uid}/entitlements/current`),resetRef=db.doc(`accountResets/${uid}`),leaseId=crypto.randomUUID();
 return db.runTransaction(async tx=>{
  const [controlDoc,entDoc,resetDoc]=await Promise.all([tx.get(ref),tx.get(entRef),tx.get(resetRef)]),accountResetEpoch=AccountWriteFence.readyEpoch(resetDoc),current=controlDoc.data(),ent=entDoc.data()||{},pending=current&&['purging','failed'].includes(current.status);
  ...
  const lastRetentionEpoch=Math.max(0,Number(ent.accountRetention&&ent.accountRetention.lastRetentionEpoch)||0);tx.set(ref,value);tx.set(entRef,{accountResetStatus:'complete',accountResetEpoch,accountResetEpochString:String(accountResetEpoch),accountRetention:{...ent.accountRetention,controlStatus:'purging',activeRetentionEpoch:epoch,lastRetentionEpoch,lastRetentionEpochString:String(lastRetentionEpoch),purgeRequired:true,requiredCutoffDate:decision.cutoffDate,retainThroughDate:decision.retainThroughDate||null}},{merge:true});return{status:'acquired',ref,epoch,accountResetEpoch,leaseId,leaseMs,decision,entitlement:ent,startedAt:value.startedAt};
 });
}
...
async function prepareJob(db,uid,lease,now){
 ...
 for(const row of cloud.entries){const retained=entries.get(row.data.id||row.id);ops.push(retained?{path:row.path,data:{...retained,accountOwnerUid:uid,retentionEpoch:lease.epoch,accountResetEpoch:lease.accountResetEpoch}}:{path:row.path,remove:true});}
 ...
 for(const [collection,rows]of Object.entries(records))for(const r of rows){const companyId=collection==='entities'?r.id:r.entityId||r.sourceTransaction&&r.sourceTransaction.beneficiaryEntityId||r.ownerType==='entity'&&r.ownerId||known.size===1&&[...known][0];const e={...Policy.envelope(collection,r,companyId,lease.epoch),accountResetEpoch:lease.accountResetEpoch},path=`users/${uid}/ltd/v1/${collection}/${e.documentId}`;retainedPaths.add(path);ops.push({path,data:e});}
 for(const r of cloud.ltd)if(!retainedPaths.has(r.path))ops.push({path:r.path,remove:true});
 const meta={...cloud.meta,...Object.fromEntries(['businesses','businessTombstones','folders','folderTombstones','customCats','activeCats','yearData','settings','metaVersions'].map(k=>[k,after[k]||(['businesses','businessTombstones','folders','folderTombstones'].includes(k)?[]:{})])),retention:after.retention,retentionEpoch:lease.epoch,accountResetEpoch:lease.accountResetEpoch,accountOwnerUid:uid,updatedAt:lease.startedAt,deviceId:'server-retention'};
...
async function retentionRun({db,bucket,uid,now=()=>Date.now(),leaseMs=DEFAULT_LEASE_MS,projectId,hooks={}}){
 ...
 try{
  ...
  for(const file of files){await fenced(db,lease,now,async()=>{});try{const [metadata]=await file.getMetadata(),current=Date.parse(metadata.timeCreated)>=Date.parse(lease.decision.cutoffDate+'T00:00:00+01:00'),fencedMetadata={...metadata.metadata,retentionEpoch:String(lease.epoch),accountResetEpoch:String(lease.accountResetEpoch)};if(keep.has(file.name)||current){await file.setMetadata({metadata:fencedMetadata});keptReceipts++;}else{try{const result=await require('./receipt-cleanup').cleanupReceiptWithRetry({db,bucket,path:file.name,beforeAttempt:()=>fenced(db,lease,now,async()=>{})});if(result.status==='deleted')deletedReceipts++;else{await file.setMetadata({metadata:fencedMetadata});keptReceipts++;}}catch(error){await require('./receipt-cleanup').enqueueCleanup({db,path:file.name,retentionUid:uid,retentionEpoch:lease.epoch});pendingReceipts.push(file.name);}}}catch(error){if(Number(error.code)!==404)throw error;}if(hooks.afterReceipt)await hooks.afterReceipt(file.name);}
 ...
 }catch(error){await db.runTransaction(async tx=>{const entRef=db.doc(`users/${uid}/entitlements/current`),[c,e]=await Promise.all([tx.get(lease.ref),tx.get(entRef)]);if(ownedLease(c.data(),lease)){tx.update(lease.ref,{status:'failed',failedAt:now(),failedStage:stage,leaseId:null,leaseUntil:null,reasonCode:String(error.message||'retention_failed').replace(/[^a-z0-9_-]/gi,'_').slice(0,80)});if(e.exists){const lastRetentionEpoch=Math.max(0,Number(e.data().accountRetention&&e.data().accountRetention.lastRetentionEpoch)||0);tx.set(entRef,{accountResetStatus:'complete',accountResetEpoch:lease.accountResetEpoch,accountResetEpochString:String(lease.accountResetEpoch),accountRetention:{...e.data().accountRetention,controlStatus:'failed',activeRetentionEpoch:lease.epoch,lastRetentionEpoch,lastRetentionEpochString:String(lastRetentionEpoch),purgeRequired:true}},{merge:true});}}}).catch(()=>{});throw error;}
}
