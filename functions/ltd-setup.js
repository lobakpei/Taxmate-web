'use strict';
const crypto=require('node:crypto'),ReceiptAdmission=require('./receipt-admission'),AccountWriteFence=require('./account-write-fence');
...
function createHandlers({db,FieldValue,HttpsError,authenticate,requireTier,now=Date.now}){
 ...
 async function retention(tx,uid,expectedAccountResetEpoch){const snap=await tx.get(db.doc(`users/${uid}/retention/current`));let accountResetEpoch;try{accountResetEpoch=await AccountWriteFence.readInTransaction({tx,db,uid,expectedEpoch:expectedAccountResetEpoch});}catch(error){if(error instanceof AccountWriteFence.AccountWriteFenceError)fail('account_reset_processing');throw error;}if(snap.exists&&!['complete','complete_with_warnings'].includes(snap.data().status))fail('retention_processing');return{epoch:snap.exists?Number(snap.data().epoch)||0:0,accountResetEpoch,version:version(snap)};}
 ...
 async function claim(req){
  ...
  return db.runTransaction(async tx=>{
   const anchor=anchorRef(user.uid),[snap,outcome,control]=await Promise.all([tx.get(anchor),tx.get(outcomeRef(user.uid,companyId)),retention(tx,user.uid,req.data?.accountResetEpoch)]);
   ...
   const {epoch,accountResetEpoch}=control;
   ...
   tx.create(anchor,data);if(modern)for(const row of seed)tx.create(collectionRef(user.uid,row.collection).doc(row.documentId),{...clone(row),retentionEpoch:epoch,accountResetEpoch});
 ...
 async function inspect(tx,uid,companyId,expectedAccountResetEpoch){
  ...
  const control=await retention(tx,uid,expectedAccountResetEpoch),batches=await Promise.all(COLLECTIONS.map(async name=>({name,snapshot:await tx.get(collectionRef(uid,name).where('companyId','==',companyId).limit(401))})));
  ...
  if(!matches){if(docs.length)fail('setup_anchor_changed');return{status:'unclaimed',canDiscard:true,companyId,versionToken:token,epoch:control.epoch,accountResetEpoch:control.accountResetEpoch,docs:[]};}
  ...
  return{status:'pending',canDiscard:true,companyId,versionToken:token,epoch:control.epoch,accountResetEpoch:control.accountResetEpoch,docs,profile,entity};
 }
 async function manage(req){
  ...
  return db.runTransaction(async tx=>{
   const [receipt]=await Promise.all([tx.get(outcomeRef(user.uid,companyId)),retention(tx,user.uid,req.data?.accountResetEpoch)]);
   ...
   const state=await inspect(tx,user.uid,companyId,req.data?.accountResetEpoch);
   if(action==='inspect'){const {docs,profile,entity,epoch,accountResetEpoch,...publicState}=state;return publicState;}
