...
const FOUNDER_CLIENT_VERSION='2.1.29';
...
function createHandler({HttpsError,authenticate,requireTier,apiKey,fetchImpl=globalThis.fetch,expectedFounderUidSha256=FOUNDER_UID_SHA256,expectedFounderEmailSha256=FOUNDER_EMAIL_SHA256,requiredFounderClientVersion=FOUNDER_CLIENT_VERSION,diagnosticLog=console.warn}){
  ...
  return async req=>{
    ...
    if(isAlias){
      ...
      const clientPatch=/^2\.1\.(\d+)$/.exec(clientVersion),supportedPatch=/^2\.1\.(\d+)$/.exec(requiredFounderClientVersion);
      if(!clientPatch||!supportedPatch||Number(clientPatch[1])<15||Number(clientPatch[1])>Number(supportedPatch[1])){log('client_version','client');throw invalidCompanyNumber();}
